...
class WOO_Vou_Public {
    ...
    public function woo_vou_check_code_content() {
         ...
         <table class="form-table woo-vou-check-code">
             <tr>
                     ...
                     <h1><label for="woo_vou_voucher_code"><?php _e('Check and redeem vouchers:', 'woovoucher') ?></label></h1>
           
                 <td>
                     <input type="text" id="woo_vou_voucher_code" name="woo_vou_voucher_code" placeholder="Enter voucher code" />
                     <input type="hidden" id="woo_vou_valid_voucher_code" name="woo_vou_valid_voucher_code" value="" />
                     <input type="button" id="woo_vou_check_voucher_code" name="woo_vou_check_voucher_code" class="button-primary" value="<?php _e('Check It', 'woovoucher') ?>" data-currency="<?php echo $currency; ?>"/>
             ...
             // if ((in_array($user_role, $admin_roles) || in_array($user_role, $woo_vou_vendor_role)) || ( !empty( $vou_allow_secondary_vendor_redeem_primary_voucher ) && $vou_allow_secondary_vendor_redeem_primary_voucher == 'yes' ) || (!in_array($user_role, $admin_roles) && !in_array($user_role, $woo_vou_vendor_role) && ($vou_enable_logged_user_check_voucher_code == 'yes') && ($vou_enable_logged_user_redeem_vou_code == 'yes') ) || apply_filters('woo_vou_access_redeem_btn_without_login', false)
             // ) {// voucher admin can redeem all codes
    ...
    public function woo_vou_add_to_cart_validation($valid, $product_id = '', $quantity = '', $variation_id = '', $variations = array(), $cart_item_data = array()) {
        ...
        if ($voucher_enable) {//If voucher enable
            ...
            foreach ($recipient_columns as $recipient_key => $recipient_val) {
                ...
                $recipient_key_hide       = get_post_meta( $variation_id, $prefix.'hide_'.$recipient_key, true );
                ...
                if (array_key_exists($recipient_key, $individual_recipient_details) || empty($_delivery_method) || (!empty($_delivery_method) && $product_delivery_meth['enable_' . $_delivery_method] == 'yes' && !empty($product_delivery_meth[$_delivery_method]) && is_array($product_delivery_meth[$_delivery_method]) && in_array($recipient_key, $product_delivery_meth[$_delivery_method]) )) {
                    ...
                    if ($recipient_data['enable_' . $recipient_key] == 'yes' && $recipient_key_hide != 'yes'  && $recipient_data[$recipient_key . '_is_required'] == 'yes' && empty($_POST[$prefix . $recipient_key][$_product_id]) && ${$recipient_key . '_err_enable'} ) {
    ...
    public function woo_vou_payment_process_or_complete($order_id) {
        ...
        if( $woo_order->get_meta( $prefix . 'order_hide_voucher_data', true ) ) return;
        ...
        if ( $woo_order->get_meta( $prefix . 'recipient_email_sent', true ) ) {
            return;
        }
        ...
        if ($recipient_gift_email_send) {
            ...
            $woo_order->update_meta_data(  $prefix . 'recipient_email_sent' , true );
    ...
    */
    ...
    public function woo_vou_variation_start_enddate(){
        if( isset($_POST['variation_id'] ) && !empty($_POST['variation_id'] )){
            $datas = $this->model->woo_vou_get_minmax_date_from_product_variation($_POST['variation_id']);
            $woo_hidden_recipient_fiels = $this->model->woo_vou_get_hidden_recipient_data_from_product_variation($_POST['variation_id']);
            $datas = array_merge( $datas, $woo_hidden_recipient_fiels );
            wp_send_json($datas);
            exit;
        }
    }
    ...
    public function woo_vou_login_custom_redirect( $redirect_to, $request = '', $user = '' ) {
        ...
        if( strrpos($request_temp, 'woo_vou_code') !== false && strrpos($request_temp, 'woo_vou_direct_redeem=1') !== false ) {
            $redirect_to = $redirect_to.'&woo_vou_direct_redeem=1';
        }
    ...
    /**
     * Handle to reset woococommerce voucher coupon codes limit to used on order cancelled
     *
     * @package WooCommerce - PDF Vouchers
     * @since 4.3.14
     */
    public function woo_vou_set_voucher_codes_limit_on_order_cancelled($order_id, $old_status, $new_status) {
        //Get prefix
        $prefix = WOO_VOU_META_PREFIX;
        if ( $new_status == 'cancelled' ) { 
            ...
            foreach( $order->get_coupon_codes() as $coupon_code ) { 
                ...
                $woo_vou_coupon_type    = get_post_meta($coupon->get_ID(), $prefix.'coupon_type', true);
                $coupon_amount          = $coupon->get_amount();
                if( $woo_vou_coupon_type == 'voucher_code' && $coupon_amount == 0 ){
                    update_post_meta( $coupon->get_ID(), 'usage_count', 1 );
                }   
            }
        }
    }
    ...
    /**
     * Add QR scan buttton front side on voucher redeem page 
     *
     * @package WooCommerce - PDF Vouchers
     * @since 4.6.0
     */
    public function woo_vou_vendor_add_scan_btn_on_login_screen( $voucode ){

        $vou_enable_vendor_qr_login                 = get_option('vou_enable_vendor_qr_login');
        ...
        if( $woo_vou_guest_user_allow_redeem_voucher!=='yes' &&  $vou_enable_vendor_qr_login == 'yes' && !is_user_logged_in() ){

        ?>
            <div class="woo-vou-voucher-code-submit-wrap woo-vou-voucher-code-submit-wrap-<?php echo $voucode; ?>">
                <div class="woo-vou-qrlogin-wrap">
                    <div class="woo-vou-vendor-wl-wrp">
                        <div class="woo_vou_vendor_scan_reader woo_vou_vendor_scan_reader_<?php echo $voucode; ?>"  id="woo_vou_vendor_scan_reader_<?php echo $voucode; ?>"></div>    
                        <input type="button" data-code="<?php echo $voucode; ?>" id="woo_vou_voucher_scan_<?php echo $voucode; ?>" name="woo_vou_voucher_scan" class="woo_vou_voucher_scan button-primary" value="<?php echo  esc_html__( "Click here to scan your QR code", "woovoucher" ); ?>"/>
                        <span class="woo-vou-vendor-qr-login-error" style="display:none;"></span>
                        <div class="woo-vou-loader woo-vou-voucher-scan-loader">
                            <img src="<?php echo esc_url( WOO_VOU_IMG_URL ); ?>/ajax-loader.gif"/>
                        </div>
                        <div class="woo-vou-voucher-qrcode-msg"><?php echo esc_html__( 'If you are experiencing difficulties while scanning the QR code, ', 'woovoucher' ) ?><a class="woo-vou-show-wp-login-form" href="javascript:void();"><?php echo esc_html__( 'please click here.', 'woovoucher' ) ?> </a></div>         
                    </div>
                </div>
            </div>
        <?php }
     ...
     * @since 4.6.0
     */
    public function woo_vou_vendor_add_scan_btn_on_redeem_screen( $voucode ){
        ...
        ?>
            <tr class="woo-vou-voucher-code-submit-wrap woo-vou-voucher-code-submit-wrap-<?php echo $voucode; ?>">
                <td class="woo-vou-qrlogin-wrap">
                    <div class="woo-vou-vendor-wl-wrp">
                        <div class="woo_vou_vendor_scan_reader woo_vou_vendor_scan_reader_<?php echo $voucode; ?>"  id="woo_vou_vendor_scan_reader_<?php echo $voucode; ?>"></div>    
                        <input type="button" data-code="<?php echo $voucode; ?>" id="woo_vou_voucher_scan_<?php echo $voucode; ?>" name="woo_vou_voucher_scan" class="woo_vou_voucher_scan button-primary" value="<?php echo  esc_html__( "Click here to scan your QR code", "woovoucher" ); ?>"/>
                        <span class="woo-vou-vendor-qr-login-error" style="display:none;"></span>
    ...
    /**
     * Login using vendor QR code 
     *
     * @package WooCommerce - PDF Vouchers
     * @since 4.6.0
     */    
    public function woo_vou_vendor_login_using_qrcode() {
        global $woo_vou_vendor_role;
        $vou_enable_vendor_qr_login  = get_option('vou_enable_vendor_qr_login');

        if ($vou_enable_vendor_qr_login !== 'yes') {
            $response['status'] = 0;
            $response['msg'] = esc_html__('QR Login is disabled.', 'woovoucher');
            echo json_encode($response);
            die();
        }

        if (!isset($_POST['security']) || !wp_verify_nonce($_POST['security'], 'woo_vou_check_qrcode_nonce')) {
            $response['status'] = 0;
            $response['msg'] = esc_html__('Nonce verification failed.', 'woovoucher');
            echo json_encode($response);
            die();
        }

        $response = array();
        $qr_data = !empty($_POST['woo_vou_scaned_code']) ? $_POST['woo_vou_scaned_code'] : '';
        $qr_data = json_decode(woo_vou_decrypt($qr_data), true);

        if (is_array($qr_data) && isset($qr_data['user_id']) && isset($qr_data['token'])) {
            $user_id = $qr_data['user_id'];
            $token = $qr_data['token'];
            $current_user = get_user_by('id', $user_id);

            if (!empty($current_user) && in_array(WOO_VOU_VENDOR_ROLE, $current_user->roles)) {
                $stored_token = get_user_meta($user_id, '_woo_vou_vendor_token', true);
                ...
                if ( !empty( $stored_token ) && $token === $stored_token ) {
                    ...
                    $response['status'] = 1;
                    $response['msg'] = esc_html__('Success', 'woovoucher');
                } else {
                    $response['status'] = 0;
                    $response['msg'] = esc_html__('Invalid QR code or you are not a valid vendor.', 'woovoucher');
                }
            } else {
                $response['status'] = 0;
                $response['msg'] = esc_html__('Invalid QR code or you are not a valid vendor.', 'woovoucher');
            ...
            $response['status'] = 0;
            $response['msg'] = esc_html__('Invalid QR code data.', 'woovoucher');
        }
    ...
    /**
     * Generator vendor login QR code
     *
     * @package WooCommerce - PDF Vouchers
     * @since 4.6.0
     */    
    public function woo_vou_vendor_login_qrcode_generator() {
       if(  !empty( $_GET['woo_vou_vendor_qrlogin_code'] ) ){
            require_once( WOO_VOU_DIR . '/includes/tcpdf/tcpdf_barcodes_2d.php');
            $barcodeobj = new TCPDF2DBarcode($_GET['woo_vou_vendor_qrlogin_code'], 'QRCODE,H');
            $barcodeobj->getBarcodePNG(10, 10, array(0,0,0));
            exit;
       }
    }
    ...
    public function woo_vou_product_price_input() {

        global $product;
        $vou_product_id         = $product->get_id();        
        $prefix                 = WOO_VOU_META_PREFIX; // Get prefix
        $vou_enabel_pay_your_price  = get_post_meta($vou_product_id, $prefix . 'enabel_pay_your_price', true);

        if ('yes' !== $vou_enabel_pay_your_price) return;
        
        $vou_pay_your_price_min_price_label         = get_option('vou_pay_your_price_min_price_label');
        $vou_pay_your_price_max_price_label         = get_option('vou_pay_your_price_max_price_label');
        $vou_pay_your_price_recommended_price_label = get_option('vou_pay_your_price_recommended_price_label');
        $vou_pay_your_price_label                   = get_option('vou_pay_your_price_label');
        $vou_pay_your_min_price                     = get_post_meta($vou_product_id, $prefix . 'woo_vou_pay_your_min_price', true);
        $vou_pay_your_max_price                     = get_post_meta($vou_product_id, $prefix . 'woo_vou_pay_your_max_price', true);
        $vou_pay_your_recomm_price                  = get_post_meta($vou_product_id, $prefix . 'woo_vou_pay_your_recommended_price', true);
        $vou_hide_pay_your_min_price                = get_post_meta($vou_product_id, $prefix . 'woo_vou_hide_pay_your_min_price', true);
        $vou_hide_pay_your_max_price                = get_post_meta($vou_product_id, $prefix . 'woo_vou_hide_pay_your_max_price', true);
        $vou_pay_your_price_desc_label              = '';
        $vou_pay_your_price_input_field_type        = get_post_meta($vou_product_id, $prefix . 'woo_vou_pay_your_price_input_field_type', true);
        $vou_pay_your_price_input_field_type_amount_values  = get_post_meta($vou_product_id, $prefix . 'woo_vou_pay_your_price_input_field_type_amount_values', true);

        if ($vou_pay_your_price_recommended_price_label != '' && $vou_pay_your_recomm_price != '') {
            echo '<div class="vou_pay_your_price_labels_wrap"><div class="vou_pay_your_price_label vou_pay_your_price_recommended_price_label">' 
                . str_replace('{price}', get_woocommerce_currency_symbol() . $vou_pay_your_recomm_price, $vou_pay_your_price_recommended_price_label) 
                . '</div></div>';
        }

        woocommerce_form_field('vou_set_pay_your_price', array(
            'type'          => 'text',
            'label_class'   => array('vou_set_pay_your_price'),
            'required'      => true,
            'label'         => str_replace('{currency}', get_woocommerce_currency_symbol(), $vou_pay_your_price_label),            
            'default'       => $vou_pay_your_recomm_price != '' ? $vou_pay_your_recomm_price : '',
        ));

        if( $vou_pay_your_price_input_field_type != '' && $vou_pay_your_price_input_field_type != 'text' && $vou_pay_your_price_input_field_type_amount_values != '' ){
            $vou_pay_your_price_input_field_type_amount_value = explode( ',', $vou_pay_your_price_input_field_type_amount_values );
            if( !empty( $vou_pay_your_price_input_field_type_amount_value ) ){
                echo '<div class="vou_pay_your_price_labels_wrap">';
                foreach( $vou_pay_your_price_input_field_type_amount_value as $button_price ){
                    $button_price = preg_replace("/[^0-9\.]/", "", $button_price);
                    if(  $button_price != '' ){
                        echo '<button data-vou_pay_your_amount="'.$button_price.'" type="button" class="button vou_pay_your_amount_button">'.get_woocommerce_currency_symbol().$button_price.'</button>';
                    }
                }
                echo '</div>';
            }
        ...
        if ($vou_pay_your_price_min_price_label != '' && $vou_hide_pay_your_min_price != 'yes' && $vou_pay_your_min_price != '') {
            echo '<div class="vou_pay_your_price_labels_wrap"><div class="vou_pay_your_price_label vou_pay_your_price_min_price_label">' 
                . str_replace('{price}', get_woocommerce_currency_symbol() . $vou_pay_your_min_price, $vou_pay_your_price_min_price_label) 
                . '</div></div>';
        }

        if ($vou_pay_your_price_max_price_label != '' && $vou_hide_pay_your_max_price != 'yes' && $vou_pay_your_max_price != '') {
            echo '<div class="vou_pay_your_price_labels_wrap"><div class="vou_pay_your_price_label vou_pay_your_price_max_price_label">' 
                . str_replace('{price}', get_woocommerce_currency_symbol() . $vou_pay_your_max_price, $vou_pay_your_price_max_price_label) 
                . '</div></div>';
        }
    }
    ...
    /**
     * Pay your price
     * Add validation based on pay your price input field's value
     *
     * @package WooCommerce - PDF Vouchers
     * 
     ...
     * @return void
     */
    public function woo_vou_product_add_on_validation( $passed, $product_id, $qty ) {
        $prefix                     = WOO_VOU_META_PREFIX; // Get prefix
        $vou_enabel_pay_your_price  = get_post_meta( $product_id,  $prefix . 'enabel_pay_your_price', true );

        if ( 'yes' !== $vou_enabel_pay_your_price ) return $passed;

        $vou_set_pay_your_price                     = isset( $_POST['vou_set_pay_your_price'] ) && sanitize_text_field( $_POST['vou_set_pay_your_price'] ) != '' ? sanitize_text_field( $_POST['vou_set_pay_your_price'] ) : '';
        $vou_pay_your_min_price                     = get_post_meta( $product_id,  $prefix . 'woo_vou_pay_your_min_price', true );
        $vou_pay_your_max_price                     = get_post_meta( $product_id,  $prefix . 'woo_vou_pay_your_max_price', true );
        $vou_pay_your_price_input_error_message     = get_option('vou_pay_your_price_input_error_message');
        $vou_pay_your_price_min_price_error_message = get_option('vou_pay_your_price_min_price_error_message');
        $vou_pay_your_price_max_price_error_message = get_option('vou_pay_your_price_max_price_error_message');

        if ( $vou_set_pay_your_price == '' ) {
            wc_add_notice( $vou_pay_your_price_input_error_message, 'error' );
            $passed = false;
        }else if ( !is_numeric( $vou_set_pay_your_price ) ) {
            wc_add_notice( __( 'Please enter a valid amount', 'woovoucher' ), 'error' );
            $passed = false;
        }elseif( $vou_pay_your_min_price != '' && $vou_set_pay_your_price < $vou_pay_your_min_price ){            
            wc_add_notice( str_replace( '{price}', get_woocommerce_currency_symbol() . $vou_pay_your_min_price, $vou_pay_your_price_min_price_error_message), 'error' );
            $passed = false;
        }elseif( $vou_pay_your_max_price != '' && $vou_set_pay_your_price > $vou_pay_your_max_price ){
            wc_add_notice( str_replace( '{price}', get_woocommerce_currency_symbol() . $vou_pay_your_max_price, $vou_pay_your_price_max_price_error_message), 'error' );
            $passed = false;
        }
        return $passed;
    }

    public function woo_vou_product_add_on_cart_item_data( $cart_item, $product_id ) {
        $prefix                     = WOO_VOU_META_PREFIX; // Get prefix
        $vou_enabel_pay_your_price  = get_post_meta( $product_id,  $prefix . 'enabel_pay_your_price', true );

        if ( 'yes' !== $vou_enabel_pay_your_price ) return $cart_item;  
        
        $cart_item['vou_set_pay_your_price'] = sanitize_text_field( $_POST['vou_set_pay_your_price'] );

        
        return $cart_item;
    }

    function woo_vou_alter_price_cart( $cart ) {
        $prefix                     = WOO_VOU_META_PREFIX; // Get prefix
        ...
        foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
           ...
           $vou_enabel_pay_your_price  = get_post_meta( $product->get_id(),  $prefix . 'enabel_pay_your_price', true );

            if ( 'yes' !== $vou_enabel_pay_your_price ){
              continue;  
            }

            $cart_item['data']->set_price( $cart_item['vou_set_pay_your_price'] );
        } 
    }

    public function woo_vou_remove_price( $price, $product  ){
        ...
        $prefix                     = WOO_VOU_META_PREFIX; // Get prefix
        $vou_enabel_pay_your_price  = get_post_meta(  $product_id,  $prefix . 'enabel_pay_your_price', true );
        $woo_vou_pay_your_recommended_price  = get_post_meta(  $product_id,  $prefix . 'woo_vou_pay_your_recommended_price', true );
        $woo_vou_pay_your_min_price  = get_post_meta(  $product_id,  $prefix . 'woo_vou_pay_your_min_price', true );
        
        if ( 'yes' == $vou_enabel_pay_your_price ){
            if( !empty( $woo_vou_pay_your_recommended_price ) ){
                return wc_price( $woo_vou_pay_your_recommended_price );    
            } else if( !empty( $woo_vou_pay_your_min_price ) ){
                return wc_price( $woo_vou_pay_your_min_price );    
            } else {
                return $price;                
            }   
        }
    ...
    public function add_hooks() {
        ...
        add_action('woocommerce_store_api_checkout_update_order_meta', 'woo_vou_purchase_store_api_product_purchase');

        //WooPayments plugin support.
        add_action( 'woocommerce_order_status_completed', 'woo_vou_product_purchase_woopay' );
        add_action( 'woocommerce_order_status_processing', 'woo_vou_product_purchase_woopay' );
        ...
        //Check multiple vouchers ajax call
        add_action('wp_ajax_woo_vou_check_multiple_voucher_code', array($this->voucher, 'woo_vou_check_multiple_voucher_code'));
        add_action('wp_ajax_nopriv_woo_vou_check_multiple_voucher_code', array($this->voucher, 'woo_vou_check_multiple_voucher_code'));
        ...
        add_filter('woocommerce_add_cart_item_data', 'woo_vou_woocommerce_add_cart_item_data', 30, 3);
        ...
        add_action('wp_loaded', 'woo_vou_check_qrcode');
       ...
       // Filter to hide download link from the checkout and thank you page
        ...
        add_action('woocommerce_order_status_changed', array($this,'woo_vou_set_voucher_codes_limit_on_order_cancelled'), 10, 3);

        // add action for scan button on redeem page
        add_action( 'woo_vou_check_qrcode_bottom', array( $this, 'woo_vou_vendor_add_scan_btn_on_redeem_screen' ) , 10, 1 );
        // add action for scan button on login screen
        add_action( 'woo_vou_login_with_qrcode', array( $this, 'woo_vou_vendor_add_scan_btn_on_login_screen' ) , 10, 1 );

        add_action('wp_ajax_nopriv_woo_vou_vendor_login_using_qrcode', array($this, 'woo_vou_vendor_login_using_qrcode') );
        add_action('wp_ajax_woo_vou_vendor_login_using_qrcode', array($this, 'woo_vou_vendor_login_using_qrcode') );

        add_action( 'init', array( $this, 'woo_vou_vendor_login_qrcode_generator' ) );

        /**
         * Pay your price
         */
        
         // Add action for add pay your price input field before cart button 
        add_action( 'woocommerce_before_add_to_cart_button', array( $this, 'woo_vou_product_price_input' ), 9 );
        // Add action for add validation based on pay your price input field's value
        add_filter( 'woocommerce_add_to_cart_validation', array( $this, 'woo_vou_product_add_on_validation' ), 9999, 3 );
        add_filter( 'woocommerce_add_cart_item_data', array( $this, 'woo_vou_product_add_on_cart_item_data' ), 9999, 2 );
        add_action( 'woocommerce_before_calculate_totals', array( $this, 'woo_vou_alter_price_cart' ), 9999 ); 
        add_filter( 'woocommerce_get_price_html', array( $this, 'woo_vou_remove_price' ),10,2 );
    }
}
