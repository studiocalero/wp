...
class WOO_Vou_Scripts {
	...
	public function woo_vou_popup_scripts( $hook_suffix ) {
		...
		wp_register_script( 'woo-vou-dashboard-script', WOO_VOU_URL . 'includes/js/woo-vou-dashboard.js', array(), WOO_VOU_PLUGIN_VERSION, true);
		wp_localize_script( 'woo-vou-dashboard-script', 'woovoudashboard', array(	
																					'woo_vou_nonce'	=> wp_create_nonce( 'woo_vou_check_security' ),
																				) );
		...
		//Check pages when you needed 
  		if( in_array( $hook_suffix, array( $wc_vou_vendor_screen, $wc_screen_id.'_page_woo-vou-codes', $wc_screen_id.'_page_wc-settings', $wc_screen_id.'_page_woo-vou-check-voucher-code', $woo_vou_screen_id.'_page_woo-vou-check-voucher-code', 'toplevel_page_woo-vou-check-voucher-code', 'user-edit.php' , 'user-new.php', 'profile.php', 'post.php', 'post-new.php' ) ) ) {
			...
			wp_localize_script( 'woo-vou-admin-script' , 'WooVouAdminSetOpt' , array('is_partial_option'	=> $is_partial_redeem ) );
	...
	public function woo_vou_popup_styles( $hook_suffix ) {
		...
		$pages_hook_suffix = array( 'post.php', 'post-new.php', $wc_screen_id.'_page_woo-vou-codes', 'toplevel_page_woo-vou-codes', $wc_screen_id.'_page_wc-settings', $wc_screen_id.'_page_woo-vou-check-voucher-code' );
		...
		if( in_array( $hook_suffix, $pages_hook_suffix ) ) {
			...
			//Check vouchers & product post type
			if( in_array( $hook_suffix, array( $wc_screen_id . '_page_woo-vou-codes', 'toplevel_page_woo-vou-codes', $wc_screen_id.'_page_wc-settings', $wc_screen_id.'_page_woo-vou-check-voucher-code' ) )
				|| ( isset( $post->post_type ) && $post->post_type == WOO_VOU_MAIN_POST_TYPE ) ) {
	...
	public function woo_vou_metabox_scripts() {
		...
		wp_register_script( 'woo-vou-yith-backend-script', WOO_VOU_URL . 'includes/js/woo_vou_yith_backend_compatibility.js', array(), WOO_VOU_PLUGIN_VERSION, true);
	...
	public function woo_vou_public_scripts(){
		...
		wp_register_script('sweetalert-script', WOO_VOU_URL . 'includes/js/sweetalert2.all.min.js', array('jquery'), WOO_VOU_PLUGIN_VERSION, true); 
		...
		if(  has_shortcode( $post_content, 'woo_vou_check_code' ) || $is_archive || apply_filters( 'woo_vou_enqueue_check_code_script', false ) ) {
			...
			wp_localize_script( 'woo-vou-check-code-script' , 'WooVouCheck' , array( 
																						...
																						'check_multi_code_error' 	=> esc_html__( 'Please enter voucher code(s).', 'woovoucher' ),
																						'check_multi_code_select_atleast_one_error' 	=> esc_html__( 'Please select atleast one voucher code.', 'woovoucher' ),
																						'check_multi_code_redeem_all_title_error' 	=> esc_html__( 'Are you sure?', 'woovoucher' ),
																						'check_multi_code_redeem_all_text_error' 	=> esc_html__( 'You want to redeem all selected voucher codes? They will be redeemed at full price.', 'woovoucher' ),
		...
		if( has_shortcode( $post_content, 'woo_vou_check_code' ) || has_shortcode( $post_content, 'woo_vou_used_voucher_codes' ) || has_shortcode( $post_content, 'woo_vou_purchased_voucher_codes' ) 
		...
		if(isset($_GET['woo_vou_code'])){
			...
			wp_register_script( 'woo-vou-html5-qrcode', WOO_VOU_URL . 'includes/js/woo-vou-html5-qrcode.min.js', array(), WOO_VOU_PLUGIN_VERSION );
			wp_enqueue_script( 'woo-vou-html5-qrcode' );
			...
			wp_localize_script( 'woo-vou-check-qrcode' , 'WooVouQRCode' , array( 
			                                                                            'ajaxurl' 					=> admin_url( 'admin-ajax.php', ( is_ssl() ? 'https' : 'http' ) ),
			                                                                            'enabled_qr_login' 			=> $vou_enable_vendor_qr_login,
			                                                                            'woo_vou_direct_redeem' 	=> !empty( $_GET['woo_vou_direct_redeem'] ) ? 1 : 0,
			                                                                            'security' 					=> wp_create_nonce('woo_vou_check_qrcode_nonce'),
	...
	public function woo_vou_check_code_scripts( $hook_suffix ){
		...
		wp_enqueue_script( 'sweetalert-script', WOO_VOU_URL . 'includes/js/sweetalert2.all.min.js', array( 'jquery' ), WOO_VOU_PLUGIN_VERSION, true );
	}
    ...
    public function woo_vou_setting_page_scripts( $hook_suffix ){
		...
		if( $hook_suffix == 'dashboard_page_woo-vou-upgrades-voucher') {
			...
			if( isset($_GET['woo-vou-upgrade']) && $_GET['woo-vou-upgrade'] == 'migrate-voucher-partial-redeem-date' ){

				wp_register_script( 'woo-vou-admin-redeem-date-vouchers-script', WOO_VOU_URL . 'includes/js/woo-vou-admin-redeem-date-vouchers.js', array('jquery'));

				wp_localize_script( 'woo-vou-admin-redeem-date-vouchers-script' , 'WooVouUpgrd' , array( 
					'ajaxurl'	=> admin_url( 'admin-ajax.php', ( is_ssl() ? 'https' : 'http' ) ),
					'security' => wp_create_nonce('woo_vou_check_security'),
				));

				wp_enqueue_script( 'woo-vou-admin-redeem-date-vouchers-script' );
    ...
    */
	...
	public function woo_vou_remove_all_theme_scripts() {
		...
		if( isset($_GET['woo_vou_code']) ){
			...
			$allow = apply_filters("woo_vou_allow_script_qr_page",array('woo-vou-check-qrcode','woo-vou-html5-qrcode'));	
			global $wp_scripts;						
