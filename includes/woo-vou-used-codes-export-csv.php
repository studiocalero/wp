...
function woo_vou_code_export_to_csv(){
	...
	if( !empty( $_GET['woo-vou-voucher-exp-csv'] ) && $_GET['woo-vou-voucher-exp-csv'] == '1' )  {
		...
		if( isset( $_GET['woo_vou_action'] ) && $_GET['woo_vou_action'] == 'used' ) {
			...
			if ( 'partial' !== $woo_vou_partial_used_voucode && 'yes' !== $woo_vou_partial_used_voucode ) {
					$args['meta_query'] = array( array( 
						'key' 		=> $prefix . 'used_codes',
						'value' 	=> '',
						'compare' 	=> '!='
			...
			if( $woo_vou_partial_used_voucode !== 'partial' && $woo_vou_partial_used_voucode !== 'yes'){
			...
			if( !empty( $woo_vou_partial_used_voucode ) && ( $woo_vou_partial_used_voucode == 'partial' || $woo_vou_partial_used_voucode == 'yes' ) ){



				$date_meta = array();
				$starting_date = !empty($_GET['woo_vou_start_date'])?$_GET['woo_vou_start_date']:'';
				$ending_date = !empty($_GET['woo_vou_end_date'])?$_GET['woo_vou_end_date']: $woo_vou_model->woo_vou_current_date('Y-m-d');
			...
			$args = apply_filters('woo_vou_used_codes_gen_csv', $args);
		 	...
		 	if( isset( $_GET['vou-data'] ) && $_GET['vou-data'] == 'expired') {
		 	...
		 	} else if( isset( $_GET['vou-data'] ) && $_GET['vou-data'] == 'unlimited') {
				...
				$vou_file_name = 'woo-unlimited-voucher-codes-{current_date}';
	...
	if( !empty( $_GET['woo-vou-voucher-advexp-csv'] ) && $_GET['woo-vou-voucher-advexp-csv'] == '1' )  {	
		...
		$voudata		= !empty( $_GET['vou-data'] ) ? $_GET['vou-data'] : '';
		...
		if ( 'used' == $woo_vou_action ) {
			...
			if( 'partial' !== $redeemType && isset( $_GET['woo_vou_start_date'] ) && !empty( $_GET['woo_vou_start_date'] ) ) {
				$args['meta_query'] =	array(
					'relation' => 'AND',
					($args['meta_query']),
					array(
			...
			if('partial' !== $redeemType && isset( $_GET['woo_vou_end_date'] ) && !empty( $_GET['woo_vou_end_date'] ) ) {
				$args['meta_query'] =	array(
					'relation' => 'AND',
					($args['meta_query']),
					array(
		...
		$columns = array(	
			...
			esc_html__( 'Product ID', 'woovoucher' ),
			esc_html__( 'Product SKU', 'woovoucher' ),
		...
		if ( $voudata == 'unlimited' || 'partial' == $redeemType || ('used' == $woo_vou_action && empty($redeemType)) ) {
			$columns[] = esc_html__( 'Redeem Information', 'woovoucher' );
			$columns[] = esc_html__( 'Redeemable Price', 'woovoucher' );
		}
		...
		if( !empty( $voucodes ) &&  count( $voucodes ) > 0 ) {
			foreach ( $voucodes as $key => $voucodes_data ) {
				...
				$export .= '"' . $product_info['id'] . '",';
				$export .= '"' . $product_info['sku'] . '",';
				...
				$vandor_address = !empty($voucher_info['vandor_address'])?$voucher_info['vandor_address']:'';
				$expires = !empty($voucher_info['expires'])?$voucher_info['expires']:'';
				$pdf_template = !empty($voucher_info['pdf_template'])?$voucher_info['pdf_template']:'';
				
				$export .= '"' . $vandor_address . '",';
				$export .= '"' . $expires . '",';
				$export .= '"' . $pdf_template . '",';
				$export .= '"' . $recipient_info . '",';
				...
				if ( $voudata == 'unlimited' || 'partial' == $redeemType || ('used' == $woo_vou_action && empty($redeemType)) ) {	
					...
					$vou_code_total_price 	=  $woo_vou_model->woo_vou_get_product_price( $orderid, $item_id, $item );
					$vou_code_remaining_redeem_price = number_format( (float)($vou_code_total_price), 2, '.', '' );
					$vou_code_total_redeemed_price = $woo_vou_voucher->woo_vou_get_total_redeemed_price_for_vouchercode( $voucodes_data['ID'] );
					$vou_code_total_price 	=  $woo_vou_model->woo_vou_get_product_price( $orderid, $item_id, $item );
					$vou_code_remaining_redeem_price = number_format( (float)( $vou_code_total_price - $vou_code_total_redeemed_price), 2, '.', '' );
					$vou_code_remaining_redeem_price = strip_tags( wc_price($vou_code_remaining_redeem_price) );
					//remaining redeem price end
					
					$export .= '"' . html_entity_decode( $redeem_price_html ) . '",';
					$export .= '"' .  html_entity_decode( $vou_code_remaining_redeem_price ) . '",';
