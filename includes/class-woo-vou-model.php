...
class WOO_Vou_Model {
    ...
    public function woo_vou_get_all_ordered_data($orderid) {
        ...
        $data = $woo_order->get_meta( $prefix . 'meta_order_details', true );
   ...
   /**
    ...
    /**
     * Handles to remove voucher code from order item meta
     *
     * @package WooCommerce - PDF Vouchers
     * @since 3.1.0
     */
    public function woo_vou_rm_vcode_frm_order_item_meta( $order_id, $remove_that_code ) {
        ...
        // Convert the code to remove to lowercase for case-insensitive comparison
        $remove_that_code_lower = strtolower( $remove_that_code );

        // Loop through each item in the order
        foreach ( $order->get_items() as $item_id => $item ) {
            // Retrieve the current _woo_vou_codes value
            $current_codes = wc_get_order_item_meta( $item_id, $prefix . 'codes', true );
            if ( !$current_codes ) {
                continue; // Skip if there is no _woo_vou_codes value
            }

            // Convert the current codes string into an array
            $codes_array = explode( ', ', $current_codes );

            // Remove the specified code (case-insensitive)
            $updated_codes_array = array_filter( $codes_array, function( $code ) use ( $remove_that_code_lower ) {
                return strtolower( trim( $code ) ) !== $remove_that_code_lower;
            });

            if ( empty( $updated_codes_array ) ) {
                // Delete the _woo_vou_codes meta if no codes are left
                wc_delete_order_item_meta( $item_id, $prefix . 'codes' );

                //delete_post_meta( $order_id, $prefix . 'meta_order_details' );
                $order->delete_meta_data( $prefix . 'meta_order_details' );
                $order->save();
                

            } else {
                // Convert the updated codes array back into a string
                $updated_codes = implode( ', ', $updated_codes_array );

                // Update the _woo_vou_codes value
                wc_update_order_item_meta( $item_id, $prefix . 'codes', $updated_codes );
            }
        }
    ...
    /**
     * Handles to bulk delete voucher code from list
     *
     * @package WooCommerce - PDF Vouchers
     * @since 3.1.0
     */
    public function woo_vou_bulk_delete( $args = array() ) { 
       
        global $wpdb;

        if( isset( $args['purchasedvou_id'] ) && !empty( $args['purchasedvou_id'] ) ) {
            
            $voucodeid          = $args['purchasedvou_id'];
            $prefix             = WOO_VOU_META_PREFIX;
           
            $voucode_order_id   = get_post_meta($voucodeid, $prefix . 'order_id', true);
            $meta_order_details = get_post_meta($voucode_order_id, $prefix . 'meta_order_details', true);
            ...
            $this->woo_vou_rm_vcode_frm_order_item_meta($voucode_order_id, $voucode);
            
            if ($coupon->get_id()) {
                wp_delete_post($coupon->get_id(), true);
            } 

            wp_delete_post( $args['purchasedvou_id'] );
            $this->woo_vou_delete_posts_by_post_parent( $args['purchasedvou_id'] );

        }
    }

    public function woo_vou_delete_posts_by_post_parent( $parent_id ) {
        ...
        // The WP_Query to get all child posts
    ...
    public function woo_vou_get_hidden_recipient_data_from_product_variation( $variation_id ){
        $prefix = WOO_VOU_META_PREFIX;
        $woo_vou_hidden_recipient_data_fields = array();
        ...
        foreach ( $woo_vou_recipient_details_columns as $recipient_key => $recipient_val ) {
            $woo_vou_hidden_recipient_data = get_post_meta( $variation_id, $prefix.'hide_'.$recipient_key, true );
            if( $woo_vou_hidden_recipient_data == 'yes' ){
                $woo_vou_hidden_recipient_data_fields['woo_vou_hidden_recipient_fields'][] = $recipient_key.'-'.$variation_id;
            }
        }
        return $woo_vou_hidden_recipient_data_fields;
    }
}