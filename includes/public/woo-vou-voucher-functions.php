...
function woo_vou_generate_posts_from_vou_codes($allcodes, $order_id, $vou_code_post_data) {
   ...
   // $order_customer_user = get_post_meta($order_id, '_customer_user', true); //Customer ID
    ...
    $order_customer_user = $woo_order->get_customer_id();
...
function woo_vou_restore_order_voucher_codes($order_id, $called_from = '') {
    ...
    if (!empty($order_id)) {
        ...
        if (!empty($vochers)) {//If empty voucher codes
            ...
            //--HPOS--//
            //delete voucher order details
            $woo_order->delete_meta_data( $prefix . 'order_details' );

            //delete voucher order details with all meta data
            $woo_order->delete_meta_data( $prefix . 'meta_order_details' );
            ...
            if( empty( $called_from ) ) {
                ...
                $woo_order->update_meta_data( $prefix .'order_hide_voucher_data', true );
            }
...
function woo_vou_refund_order_voucher_codes($order_id) {
    ...
    if (!empty($order_id)) {
        ...
        do_action('woo_vou_after_refund_order',$order_id,$vochers );
...
function woo_vou_unlink_preview_pdf() {
    ...
    // Check if 'preview_file_name' exists and is not empty
    if (array_key_exists('preview_file_name', $_POST) && !empty($_POST['preview_file_name'])) {
        // Sanitize the input filename
        $preview_file_name = sanitize_file_name($_POST['preview_file_name']);
        
        // Prevent path traversal by ensuring the sanitized filename does not contain ".."
        if (strpos($preview_file_name, '..') === false) {
            $file_path = WOO_VOU_PREVIEW_UPLOAD_DIR . $preview_file_name;
...
// Hook the function to the appropriate action or AJAX call
