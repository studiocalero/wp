...
/**
 * Handles to generate voucher when payment made using WooPayments
 * Added this fuction because hook woocommerce_checkout_update_order_meta was not working.
 * https://wordpress.org/plugins/woocommerce-payments/
 * @package WooCommerce - PDF Vouchers
 * @since 1.0.0
 */
function woo_vou_product_purchase_woopay($order_id){
	$order = wc_get_order( $order_id );
	if ( $order->get_meta( 'is_woopay' ) ) {
		woo_vou_product_purchase($order_id);
		woo_vou_insert_downloadable_files($order_id);
		woo_vou_generate_couponcode_from_vouchercode($order_id);
	}
}

/**
 * Handles to update voucher details in order data when store API call
 * 
 * @package WooCommerce - PDF Vouchers
 * @since 1.0.0
 */
function woo_vou_purchase_store_api_product_purchase($order) {
	...
	$buyername = str_replace(',', '', $userfirstname );
	$buyername = sanitize_title_with_dashes( $buyername );
	...
	if (is_array($order_items)) {
		...
		if (!empty($vouchermetadata)) { // Check voucher meta data are not empty
			...
			//--HPOS--//
			$order->update_meta_data( $prefix . 'meta_order_details', $vouchermetadata );
			$order->save();
...
function woo_vou_update_multiple_pdf_settings( $order_id, $product_id ) {
	...
	$order_multiple_pdf = $woo_order->get_meta($prefix . 'multiple_pdf', true );  	
	...
	if( !empty( $multiple_pdf ) ) {
		...
		$woo_order->update_meta_data( $prefix . 'multiple_pdf', $order_multiple_pdf );
		$woo_order->save();
	} else {
...
function woo_vou_after_before_add_to_cart_button() {
	...
	<div class="woo-vou-fields-main-wrapper">
		<div class="woo-loder-wrap" style="display: none;">
			<img  src="<?php echo 	WOO_VOU_IMG_URL.'/ajax-loader.gif'; ?>" alt="">
		</div>
		<?php do_action('woo_vou_product_recipient_fields'); ?>
	</div>
	<?php
}
...
function woo_vou_woocommerce_get_item_data($data, $item) {
	...
	if( !in_array('woocommerce-pdf-vouchers-offline-delivery-charges-addon/woocommerce-pdf-vouchers-offline-delivery-charges-addon.php', apply_filters('active_plugins', get_option('active_plugins'))) && !empty($item[$prefix.'delivery_charge']) ) {
...
function woo_vou_attach_voucher_to_email($attachments, $status, $order, $order_item_id = false ) {
	...
	//--HPOS--//
	if( $woo_order->get_meta( $prefix . 'order_hide_voucher_data', true ) ) return $attachments;
...
function woo_vou_process_shop_order_manually($order_id) {
	...
	$original_order_status = isset( $_POST['original_order_status'] ) ? $_POST['original_order_status'] : '';
	
	// If order item are not empty
	if ( ( !empty($_POST['order_item_id']) ) && ( 'auto-draft' == $orignal_post_status || 'auto-draft' == $original_order_status ) ) {
...
function woo_vou_get_item_pdf_downloads($files, $item, $abs_order) {
	...
	$order_hide_vou = ( !empty( $abs_order ) && is_object( $abs_order ) ) ? $abs_order->get_meta( $prefix . 'order_hide_voucher_data', true ) : '';
	...
	if ( function_exists('icl_object_id') ) {
		...
		//--HPOS--//
		$wpmlLang = $abs_order->get_meta( 'wpml_language', true );
	...
	if ( is_user_logged_in() && ( $vou_download_dashboard == 'yes') ) {//If user is logged in AND download enable for customer dashboard
		...
		*/
		...
		$args = array(
            ...
            'meta_key'  	=> $prefix . 'meta_order_details',
            'meta_compare' 	=> 'EXISTS',
            'customer_id'	=> $user_id,
			...
			if ($woo_vou_voucher->woo_vou_check_enable_voucher($product_id, $variation_id, $order_args)) {//Check voucher is enabled or not
				...
				if( ( !empty( $enable_coupon_code_meta ) && $enable_coupon_code_meta == 'yes' )
					...
					foreach ($woo_vou_codes as $woo_vou_code) {
						...
						$woo_vou_voucher->woo_vou_create_wc_coupon_code($vou_code_array, $order, $product_id,$data_id);
					}
				}
			}
		}
...
function woo_vou_update_voucher_information() {
	...
	if( isset($_REQUEST['woo_vou_voucher_information_update']) && isset($_REQUEST[ 'woo_vou_order_id' ]) ){
		...
		//--HPOS--//
		$meta_order_details = $woo_update_order->get_meta( $prefix . 'meta_order_details', true );
		...
		//--HPOS--//
		$woo_update_order->update_meta_data( $prefix . 'meta_order_details', $meta_order_details );
		
		// Update meta in voucher meta
		update_post_meta($woo_vou_code_id, $prefix . 'exp_date', $woo_new_voucher_expires_date);
		update_post_meta($woo_vou_code_id, $prefix . 'start_date', $woo_new_voucher_start_date);
		...
		//--HPOS--//
		$woo_update_order->update_meta_data( $prefix . 'exp_date', $woo_vou_expires_date_meta );
		...
		//--HPOS--//
		$woo_update_order->update_meta_data( $prefix . 'start_date', $woo_vou_start_date_meta );
		
		// Update Voucher redeem days data
		//update_post_meta( $woo_vou_code_id, $prefix . 'disable_redeem_day', $disable_redeem_day );
		//--HPOS--//
		$woo_update_order->update_meta_data( $prefix . 'disable_redeem_day', $disable_redeem_day );

		$woo_update_order->save();
