...
class WOO_Vou_Voucher {
    ...
    public function woo_vou_generate_pdf_voucher($email = '', $product_id = '', $download_id = '', $order_id = '', $item_id = '') {
        ...
        if( empty( $order_id ) ){
            ...
            $multiple_pdf = $woo_order->get_meta( $prefix . 'multiple_pdf', true );  
        }
    ...
    public function woo_vou_save_partialy_redeem_voucher_code($voucodeid, $redeem_amount, $voucode, $redeemed_page, $redeemed_on = '') {
        ...
        // update redeem dates
        woo_vou_append_partial_redeem_dates( $voucodeid, $this->model->woo_vou_current_date('Y-m-d') );
     ...
     * @since 2.7.2
     */
    public function woo_vou_save_unlimited_redeem_voucher_code($voucodeid, $redeem_amount, $voucode, $redeemed_page, $redeemed_on = '') {
        ...
        // update redeem amount
        update_post_meta($voucodeid, $prefix . 'unlimited_used', 1);
        ...
        // Redeem on
        update_post_meta($unlimited_redeem_post_id, $prefix . 'redeemed_on', $redeemed_on);
		...
		if( isset($vocher_redeem_limit) && !empty($vocher_redeem_limit) ){
            ...
            error_log($voucher_used_conut .'>='. $total_used_conut);
            error_log(( $voucher_used_conut >= $total_used_conut));

            if( ( $voucher_used_conut >= $total_used_conut ) && !empty( $total_used_conut ) && $is_partial_redeem_enable != 'yes' ) {
     ...
     * @since 2.9.3
     */
    public function woo_vou_create_wc_coupon_code($voucherCode = array(), $order = '', $product_id = '',$variation_id = '') {
        ...
        if (!in_array($code, $get_coupon_titles)) {
            ...
            if ($new_coupon_id) {
                ...
                do_action('woo_vou_gen_coupon_post_meta', $new_coupon_id, $voucherCode, $order, $product_id,$variation_id);
    ...
    public function woo_vou_get_vouchers_download_key($order_id = '', $product_id = '', $item_id = '', $item = '') {
       ...
       // $multiple_pdf = empty($order_id) ? '' : get_post_meta($order_id, $prefix . 'multiple_pdf', true);
        ...
        if (!empty($order_id) && ( in_array( $vou_voucher_delivery_type, apply_filters('woo_vou_allow_download_link_for_delivery', array( 'email'))  ) ) ) {		
        ...
        if ((!is_user_logged_in()) && ($vou_enable_guest_user_check_voucher_code != 'yes') && $is_coupon == false  ) {
            ...
            if (!empty($voucodedata) && is_array($voucodedata)) { // Check voucher code ids are not empty
                ...
                if (!empty($voucodeid)) {					
                    ...
                    $order_customer = $cart_details->get_customer_id();
                ...
                $expiry_Date = apply_filters('woo_vou_voucher_code_exp_date', $expiry_Date, $voucodeid, $product_id );
                ...
                if( ( $voucher_used_conut >= $total_used_conut ) && !empty($total_used_conut) && $is_partial_redeem_enable != 'yes' ) {
                   ...
                   $response['success'] = apply_filters('woo_vou_voucher_code_disabled_message', esc_html__("Voucher usage limit has been reached. \n", 'woovoucher'));
                   $response['allow_redeem_expired_voucher'] = "no";
                   $response['expire'] = true;
            ...
            $response = apply_filters('woo_vou_voucher_code_check_last_response', $response, $voucodeid );   
    ...
    /**
     * Check Multiple Voucher Codes
     * 
     * Handles to check multiple voucher codes
     * is valid or invalid via ajax
     * 
     * @package WooCommerce - PDF Vouchers
     * @since 1.1.0
     */
    public function woo_vou_check_multiple_voucher_code( $is_coupon = false, $via_online_coupon = false ) {
        global $woo_vou_vendor_role, $current_user,$woo_vou_wc_currency_switch, $woo_vou_voucher, $woo_vou_model, $woocommerce;	
        ...
        $voucodeid = '';
        $response['expire'] = false;
        $vou_code_args =  $used_code_args = $all_vou_codes = array();
        ...
        // Get option whether to allow secondary vendor to redeem primary vendor voucher codes
        $vou_allow_secondary_vendor_redeem_primary_voucher = get_option('vou_allow_secondary_vendor_redeem_primary_voucher');
        // Get option whether to allow guest user to redeem vendor voucher code
        $vou_enable_guest_user_redeem_voucher = get_option('woo_vou_guest_user_allow_redeem_voucher');
        ...
        if ((!is_user_logged_in()) && ($vou_enable_guest_user_check_voucher_code != 'yes') && $is_coupon == false  ) {
        ...
        } elseif (!empty($_POST['voucodes'])) { // Check voucher code is not empty
            //Voucher Code
            ob_start();
                ?>
                    <div class="woo_vou_product_details">
                        <h2><?php echo esc_html__( 'Voucher Information', 'woovoucher' );?></h2>
                        <table class="woo_vou_multiple_check_code_table">
                            <thead>
                                <tr>
                                    <th><input type="checkbox" id="woo_vou_multiple_code_select_all"><?php echo esc_html__( 'Voucher Code', 'woovoucher' );?></th>
                                    <th><?php echo esc_html__( 'Product Name', 'woovoucher' );?></th>
                                    <th><?php echo esc_html__( 'Voucher Price', 'woovoucher' );?></th>
                                    <th><?php echo esc_html__( 'Redeemable Price', 'woovoucher' );?></th>
                                    <th><?php echo esc_html__( 'Status', 'woovoucher' );?></th>
                                    <th><?php echo esc_html__( 'Action', 'woovoucher' );?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    $voucodes = explode( ',', $_POST['voucodes'] );
                                    foreach ($voucodes as $voucode) {
                                        ...
                                        // this always return array
                                        $voucodedata= woo_vou_get_voucher_details(apply_filters('woo_vou_get_primary_vendor_purchase_voucode_args', $vou_code_args));
                                        $all_vou_codes[$voucode] = isset( $voucodedata[0] ) ? $voucodedata[0] : 0 ;
                                    }
                                    foreach ($all_vou_codes as $vou_code => $vou_code_id) {
                                            $vou_multiple_code_status   = $this->woo_vou_get_voucher_code_status( $vou_code );
                                            $vou_code_action            = '---';
                                            $vou_code_action_show       = false;
                                            $vou_code_check_box         = '<input type="checkbox" class="woo_vou_multiple_redeem_code_check_box" nane="woo_vou_multiple_redeem_code[]" value="'.$vou_code_id.'">';
                                            $vou_status_tooltip_message = esc_html__('Voucher code doesn\'t exist.', 'woovoucher');
                                            $vou_before_start_date = $vou_disable_redeem_days = false;
                                            
                                            if ( !empty($vou_code_id) && $vou_code_id != 0 ){
                                            ...
                                            switch ($vou_multiple_code_status) {
                                                case 'purchased':
                                                    $vou_code_status = esc_html__( 'Valid', 'woovoucher' );                                                    
                                                    if (( ( $vou_enable_logged_user_check_voucher_code == 'yes' ) && ( $vou_enable_logged_user_redeem_vou_code == 'yes' ) && (!in_array($user_role, $admin_roles) ) && (!in_array($user_role, $woo_vou_vendor_role) ) && ( $order_customer != $current_user_id ) && ($vou_enable_guest_user_redeem_voucher != 'yes')) ) { 
                                                        $vou_code_check_box = '';
                                                    }else{
                                                        $vou_code_action   = '<div class="woo-vou-single-redeem-wrap"><button type="button" class="woo-vou-code-redeem woo-vou-action-button" data-voucherid="'.$vou_code_id.'" title="' . esc_html__('Redeem', 'woovoucher') . '">' . esc_html__('Redeem', 'woovoucher') . '</button><div class="woo-vou-loader woo-vou-check-voucher-code-loader"><img src="'.esc_url(WOO_VOU_IMG_URL).'/ajax-loader.gif"></div></div>';
                                                    }
                                                    break;

                                                case 'used':
                                                    $vou_code_status = ucfirst($vou_multiple_code_status);
                                                    $vou_code_check_box = '';
                                                    break;
                                                ...
                                                case 'invalid':
                                                    $vou_code_status = esc_html__( 'Doesn\'t exist', 'woovoucher' );
                                                    $vou_code_check_box = ''; 
                                                    $vou_status_tooltip_message = esc_html__('Voucher code doesn\'t exist.', 'woovoucher');
                                                    break;           
                                            ...
                                            $product_name = $vou_code_price = $vou_code_redeemable_price = '---';

                                            if ( !empty($vou_code_id) && $vou_code_id != 0 ){
                                                
                                                //get vouchercodes data 
                                                $voucher_data = get_post($vou_code_id);
                                                $order_id = get_post_meta($vou_code_id, $prefix . 'order_id', true);   
                                                ...
                                                foreach ($order_items as $item_id => $download_data) {
                                                    ...
                                                    if (in_array($vou_code, $voucher_codes)) {
                                                    ...
                                                    //voucher start date
                                                    $start_Date = get_post_meta($vou_code_id, $prefix . 'start_date', true);
                                                    //voucher expired date
                                                    $expiry_Date = get_post_meta($vou_code_id, $prefix . 'exp_date', true);
                                                    $expiry_Date = apply_filters('woo_vou_voucher_code_exp_date', $expiry_Date, $vou_code_id, $product_id );
                                                    
                                                    if (!in_array($user_role, $admin_roles) && !in_array($user_role, $woo_vou_vendor_role) && ($vou_enable_logged_user_check_voucher_code == 'yes') && ( ($vou_enable_logged_user_redeem_vou_code == 'no') || ( ($vou_enable_logged_user_redeem_vou_code == 'yes') && ( $order_customer != $current_user->ID ) ) )
                                                    ) {
                                                        $vou_status_tooltip_message = apply_filters('woo_vou_voucher_code_valid_message', sprintf(esc_html__("Voucher code is valid and this voucher code has been bought for %s.", 'woovoucher'), $product_name), $product_name, $vou_code_id);
                                                    } else {
                                                        $vou_status_tooltip_message = apply_filters('woo_vou_voucher_code_valid_message', sprintf(esc_html__("Voucher code is valid and this voucher code has been bought for %s. \nIf you would like to redeem voucher code, Please click on the redeem button below:", 'woovoucher'), $product_name), $product_name, $vou_code_id);
                                                    }                                                    

                                                    if (!empty($product_id)) {
                                                        $disable_redeem_days = get_post_meta($vou_code_id, $prefix . 'disable_redeem_day', true);
                                                        if (!empty($disable_redeem_days)) { // check days are selected					
                                                            $current_day = date('l');
                                                            if (in_array($current_day, $disable_redeem_days)) { // check current day redeem is enable or not
                                                                $message = implode(", ", $disable_redeem_days);
                                                                $vou_status_tooltip_message = apply_filters('woo_vou_voucher_code_disabled_message', sprintf(esc_html__("Sorry, voucher code is not allowed to be used on %s. \n", 'woovoucher'), $message, $product_name));
                                                                $response['allow_redeem_expired_voucher'] = "no";
                                                                $response['expire'] = true;
                                                                $vou_disable_redeem_days = true;
                                                            }
                                                        }
                                                    }
                                                    ...
                                                    if (isset($start_Date) && !empty($start_Date)) {
                                                        if ($start_Date > $this->model->woo_vou_current_date()) {
                                                            $vou_before_start_date = true;
                                                            $vou_status_tooltip_message = apply_filters('woo_vou_voucher_code_before_start_message', sprintf(esc_html__("Voucher code cannot be redeemed before %s for %s. \n", 'woovoucher'), $this->model->woo_vou_get_date_format($start_Date, true), $product_name), $product_name, $start_Date, $vou_code_id);
                                                        }
                                                    }
                                                    ...
                                                    if (isset($expiry_Date) && !empty($expiry_Date)) {
                                                        if ($expiry_Date < $this->model->woo_vou_current_date()) {
                                                            ...
                                                            if ($allow_redeem_expired_voucher == "yes")
                                                                ...
                                                                $response['allow_redeem_expired_voucher'] = "no";
                                                                $vou_status_tooltip_message = apply_filters('woo_vou_voucher_code_expired_message', sprintf(esc_html__("Voucher code was expired on %s for %s. \n", 'woovoucher'), $this->model->woo_vou_get_date_format($expiry_Date, true), $product_name), $product_name, $expiry_Date, $vou_code_id);
                                                        }
                                                    }

                                                    $voucher_used_conut = get_post_meta($vou_code_id,$prefix.'voucher_uses_count',true);
                                                    $total_used_conut = woo_vou_get_voucher_uses_limit_by_voucher_id($vou_code_id);
                                                    $is_partial_redeem_enable = get_option('vou_enable_partial_redeem');
                                                    $total_used_conut = !empty($total_used_conut) ? $total_used_conut : 0;

                                                    if( ( $voucher_used_conut >= $total_used_conut ) && !empty($total_used_conut) && $is_partial_redeem_enable != 'yes' ) {
                                                        
                                                        $vou_status_tooltip_message =  apply_filters('woo_vou_voucher_code_disabled_message', esc_html__("Voucher usage limit has been reached. \n", 'woovoucher'));
                                                        $response['allow_redeem_expired_voucher'] = "no";                                                        
                                                        $response['expire'] = true;
                                                    }
                                                    if( $vou_multiple_code_status == 'purchased' && ( ( $vou_before_start_date || $vou_disable_redeem_days ) == true ) ){
                                                        $vou_code_check_box = '';
                                                    }
                                                    if( $vou_multiple_code_status == 'used' ){
                                                        $used_code_date = get_post_meta($vou_code_id, $prefix . 'used_code_date', true);
                                                        $vou_status_tooltip_message = apply_filters('woo_vou_voucher_code_used_message', sprintf(esc_html__('Voucher code is invalid, was used on %s for %s.', 'woovoucher'), $this->model->woo_vou_get_date_format($used_code_date, true), $product_name), $product_name, $used_code_date, $vou_code_id);
                                                    }
                                                    if( $vou_multiple_code_status == 'expired' && $response['allow_redeem_expired_voucher'] == 'no'){
                                                        $vou_code_action = '---';
                                                        $vou_code_check_box = '';
                                                    }
                                                    /*** END: ToolTip ***/

                                                    $vou_code_price  = $woo_vou_model->woo_vou_get_product_price( $order_id, $item_id, $download_data );
                                                    $vou_code_total_price = $vou_code_price;
                                                    if( isset($vou_code_price) && !empty($vou_code_price)){
                                                        $vou_code_price  = wc_price( $vou_code_price );
                                                    }
                                                    
                                                    // get total redeemed price
                                                    $vou_code_total_redeemed_price = $woo_vou_voucher->woo_vou_get_total_redeemed_price_for_vouchercode( $vou_code_id );
                                                    ...
                                                    $vou_code_redeemable_price = wc_price( $vou_code_remaining_redeem_price );
                                                     ...
                                                     * If condition is satisfied than it hide redeem button else shows it
                                                     */
                                                    /* if (( ( $vou_enable_logged_user_check_voucher_code == 'yes' ) && ( $vou_enable_logged_user_redeem_vou_code == 'yes' ) && (!in_array($user_role, $admin_roles) ) && (!in_array($user_role, $woo_vou_vendor_role) ) && ( $order_customer != $current_user_id ) && ($vou_enable_guest_user_redeem_voucher != 'yes') && (!$vou_code_action_show ) ) ) { 
                                                    // this condtion to allow customers to redeem their own vouchers
                                                        $response['loggedin_guest_user'] = true; // it wont allow to show redeem button
                                                        $vou_code_check_box = '';    
                                                    } */
                                                ...
                                                $vou_code_status = esc_html__( 'Doesn\'t exist', 'woovoucher' );
                                                $vou_multiple_code_status= 'invalid';
                                                $vou_code_check_box = '';
                                            }
                                            if( $vou_code_id != 0 ){
                                                $vou_code_detail_page_url = add_query_arg( array( 'page' => 'woo-vou-codes', 'vou_code' => $vou_code_id ), admin_url('admin.php'));
                                                $vou_code_text = '<a href="'.esc_url($vou_code_detail_page_url).'" target="_blank">' . strtoupper($vou_code) .'<span class="dashicons dashicons-visibility"></span></a>';
                                            }else{
                                                $vou_code_text = strtoupper($vou_code);
                                            }
                                        ?>
                                        <tr class="woo_vou_multiple_code_row woo_vou_multiple_code_id_<?php echo $vou_code_id; ?>" data-vou_code_total_price="<?php echo $vou_code_total_price; ?>" data-vou_code_total_redeemed_price="<?php echo $vou_code_total_redeemed_price; ?>" data-vou_code_remaining_redeem_price="<?php echo $vou_code_remaining_redeem_price; ?>" data-vou_code="<?php echo $vou_code; ?>">
                                            <td><?php echo '<span class="woo_vou_row_checkbox">'.$vou_code_check_box . '</span>'.$vou_code_text; ?></td>
                                            <td><?php echo $product_name; ?></td>
                                            <td><?php echo $vou_code_price; ?></td>
                                            <td><?php echo $vou_code_redeemable_price; ?></td>
                                            <td><span class="vou_multiple_code_status_<?php echo $vou_multiple_code_status; ?>"><?php echo $vou_code_status; ?></span><?php echo wc_help_tip( $vou_status_tooltip_message, false); ?></td>
                                            <td><?php echo $vou_code_action; ?></td>
                                        </tr>
                                        <?php 
                                    }
                                ?>
            ...
            $response['multiple_code_html'] = $html;
    ...
    public function woo_vou_save_voucher_code($coupon = '', $order = '') {
        ...
        do_action('woo_vou_before_redeem_voucher_code',$_POST);
            ...
            if ($enable_partial_redeem == "yes") {
                ...
                if (isset($_POST['ajax']) && $_POST['ajax'] == true) {
                    ...
                    if ($order) {
                        ...
                        // Redeem method
                        $redeem_method = isset( $_POST['vou_redeem_method'] ) ? $_POST['vou_redeem_method'] : '';
                        
                        // Redeem Amount
                        $redeem_amount = isset( $_POST['vou_partial_redeem_amount'] ) ? $_POST['vou_partial_redeem_amount'] : '';
                ...
                if ( ( empty($allow_unlimited_redeem) || $allow_unlimited_redeem == 'no' ) && ( empty($usage_limit) || ( $usage_count + 1 ) <= $usage_limit ) || (apply_filters('woo_voucher_allow_redeem_used_code', false)) ) { 
                    ...
                    foreach ($voucodedata as $voucodeid) {
                        ...
                        if( ( ( $voucher_used_conut < $total_used_conut ) || (empty($total_used_conut)) ) || $is_partial_redeem_enable == 'yes'  ){
							...
							$this->woo_vou_save_unlimited_redeem_voucher_code($voucodeid, $redeem_amount, $voucode, $redeemed_page, $reedeemOn);
						} else{				
    ...
    public function woo_vou_change_voucher_expiry_date() {
        ...
        if ( !isset($_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'woo_vou_check_security') ) {
            $response['error_msg'] = esc_html__('Nonce verification failed.', 'woovoucher');
            $response['success'] = false;
            echo json_encode($response);
            exit();
        }
        
        if( !current_user_can( WOO_VOU_VENDOR_LEVEL ) ){
          return false;
        }
       ...
       $variation_id =   get_post_meta($voucher_id, $prefix.'vou_from_variation',true);
        ...
        $meta_order_details = $woo_order->get_meta( $prefix . 'meta_order_details', true );
        ...
        if (!empty($order_id) && !empty($voucher_parent_id)) {
            ...
            //--HPOS--//
            $woo_order->update_meta_data( $prefix . 'exp_date', $voucher_new_exp_date );
            $woo_order->update_meta_data( $prefix . 'meta_order_details', $meta_order_details );
            $woo_order->save();
    ...
    public function woo_vou_resend_gift_notification_email() {
        ...
        if ( !isset($_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'woo_vou_check_security') ) {
            $response['error'] = esc_html__('Nonce verification failed.', 'woovoucher');
        }
                ...
                if (!empty($vou_attach_gift_mail) && $vou_attach_gift_mail == 'yes') { //If attachment enable for gift notification
                    ...
                    if (!empty($attach_keys)) {//attachments keys not empty
                        foreach ($attach_keys as $attach_key) {
                            ...
                            $is_force_voucher_template = get_post_meta($product_id, $prefix .'force_product_template_use', true);

                            // If voucher pdf exist in folder
                            // if (file_exists($vou_pdf_name) && $is_force_voucher_template != 'yes' ) {
