...
function woo_vou_generate_pdf_by_html($html = '', $pdf_args = array()) {
    global $wubpro_in_use;

    do_action( 'woo_vou_generate_pdf_start', $html, $pdf_args );

    // Added condition to fix conflict with Builder Pro plugin when email pdf
    if( !empty( $wubpro_in_use ) && $wubpro_in_use == true ) {
        return false;
    }
...
// Retrieve saved font size and color from the voucher template
    $woo_vou_pdf_font_size = get_post_meta($pdf_template_id, $prefix . 'pdf_font_size', true);
    $woo_vou_pdf_font_color = get_post_meta($pdf_template_id, $prefix . 'pdf_font_color', true);
    
// Set default font size if not defined
    $woo_vou_pdf_font_size = !empty($woo_vou_pdf_font_size) ? intval($woo_vou_pdf_font_size) : 12;
    $woo_vou_pdf_font_color = !empty($woo_vou_pdf_font_color) ? $woo_vou_pdf_font_color : '#000000';
...
public function Footer() {
                    // Get WooCommerce store address details
                    $store_address      = get_option('woocommerce_store_address');
                    $store_address2     = get_option('woocommerce_store_address_2');
                    $store_city         = get_option('woocommerce_store_city');
                    $store_postcode     = get_option('woocommerce_store_postcode');
                    $store_country_code = get_option('woocommerce_store_country');
                    $store_state_code   = get_option('woocommerce_store_state');
                
                    // Initialize WooCommerce Countries class
                    if (class_exists('WC_Countries')) {
                        $wc_countries = new WC_Countries();
                        $store_country = isset($wc_countries->countries[$store_country_code]) ? $wc_countries->countries[$store_country_code] : $store_country_code;
                        $store_state   = isset($wc_countries->states[$store_country_code][$store_state_code]) ? $wc_countries->states[$store_country_code][$store_state_code] : $store_state_code;
                    } else {
                        $store_country = $store_country_code;
                        $store_state   = $store_state_code;
                    }
                
                    // Build full store address
                    $full_store_address = $store_address;
                    if (!empty($store_address2)) {
                        $full_store_address .= ', ' . $store_address2;
                    }
                    if (!empty($store_city)) {
                        $full_store_address .= ', ' . $store_city;
                    }
                    if (!empty($store_state)) {
                        $full_store_address .= ', ' . $store_state;
                    }
                    if (!empty($store_postcode)) {
                        $full_store_address .= ' ' . $store_postcode;
                    }
                    if (!empty($store_country)) {
                        $full_store_address .= ', ' . $store_country;
                    }
                
                    // Get business website; fallback to site URL if empty
                    $business_website = get_option('business_website');
                    if (!empty($business_website)) {
                        $business_website = preg_replace('/^(https?:\/\/)/', '', $business_website);
                        $business_website = rtrim($business_website, '/');
                    } else {
                        $business_website = preg_replace('/^(https?:\/\/)/', '', get_site_url());
                        $business_website = rtrim($business_website, '/');
                    }
                
                    // If both are empty, display nothing
                    if (empty($full_store_address) && empty($business_website)) {
                        return;
                    }
                
                    // Build footer text
                    $footer_text = '';
                    if (!empty($full_store_address)) {
                        $footer_text .= $full_store_address;
                    }
                    if (!empty($business_website)) {
                        if (!empty($footer_text)) {
                            $footer_text .= " | ";
                        }
                        $footer_text .= $business_website;
                    }
                
                    // Position the footer and output the text.
                    // Notice there's no call to SetFont(), so the current font settings are inherited.
$this->SetFont($pdf_font, '', $woo_vou_pdf_font_size); // Apply font settings
$this->SetTextColorArray(woo_vou_hex_2_rgb($woo_vou_pdf_font_color)); // Apply font color
$this->SetY(-30); // Position 30mm from the bottom
$this->Cell(0, 10, $footer_text, 0, false, 'C', 0, '', 0, false, 'T', 'M');
                ...
                public function Header() {
                    ...
                    // Retrieve custom font settings
                    $woo_vou_template_font_size = get_post_meta($woo_vou_template_id, $prefix . 'pdf_font_size', true);
                    $woo_vou_template_font_color = get_post_meta($woo_vou_template_id, $prefix . 'pdf_font_color', true);
                    
                    // Ensure font size has a default fallback
                    $font_size = !empty($woo_vou_template_font_size) ? $woo_vou_template_font_size : 12;
                    
                    // Ensure font color has a default fallback
                    $font_color = !empty($woo_vou_template_font_color) ? $woo_vou_template_font_color : '#000000';
...
// Apply the font size and color to the PDF
...
function woo_vou_process_product_pdf($productid, $orderid, $item_id = '', $orderdvoucodes = array(), $pdf_args = array()) {
    ...
    if (!empty($productid) && !empty($orderid)) { // Check product id & order id are not empty
        ...
        //vou logo
        $voulogo = isset($allvoucherdata['vendor_logo']) ? $allvoucherdata['vendor_logo']['src'] : '';
        $voulogo = ( empty( $voulogo ) && !empty( get_option('vou_vendor_default_logo') ) ) ? get_option('vou_vendor_default_logo') : $voulogo;
        ...
        if (!empty($voulogo)) {
	        ...
	        if (!empty($vou_relative_path_option) && $vou_relative_path_option == 'yes') {
	            ...
	            $vou_site_attachment_id = $model->woo_vou_get_attachment_id_from_url($voulogo); // Get attachment id
	            $image_size = @getimagesize($voulogo);          // Get Image size from URL
	            $voulogo = get_attached_file($vou_site_attachment_id);      // Get relative path
	        } else {
	
	            $voulogo = isset($voulogo) && !empty($voulogo) ? $voulogo : '';
	        }
        ...
        $is_force_voucher_template = get_post_meta($productid, $prefix .'force_product_template_use', true);

        if( $is_force_voucher_template == 'yes' && !is_array($allvoucherdata['pdf_template']) ) {
            $pdf_template_id = get_post_meta($productid, $prefix . 'pdf_template', true);
        }
...
function woo_vou_process_product_pdf_preview( $post_arr ) {
    ...
    if ( !empty( $product_id ) ) { // Check product id & order id are not empty
        ...
        //Get voucher codes
        $voucher_codes = implode(', ', array_fill(0, $quantity, 'XXX-XXX-XXXXX') );
       ...
       // $start_date = isset($allvoucherdata['start_date']) ? $allvoucherdata['start_date'] : '';
        ...
        if (!empty($voulogo)) {
	        ...
	        if (!empty($vou_relative_path_option) && $vou_relative_path_option == 'yes') {
	            ...
	            $vou_site_attachment_id = $woo_vou_model->woo_vou_get_attachment_id_from_url($voulogo); // Get attachment id
	            $image_size = @getimagesize($voulogo);          // Get Image size from URL
	            $voulogo = get_attached_file($vou_site_attachment_id);      // Get relative path
	        } else {
