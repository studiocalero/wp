...
function woo_vou_get_voucher_price( $item_id, $product_id, $values = array() ) {
	...
	if( !empty( $price_options ) && 'yes' !== $vou_enabel_pay_your_price ) {
...
/**
 * WooCommerce - get recipient data
 *
 * @package WooCommerce - PDF Vouchers
 * @since 3.7.0
 */
function woo_vou_get_recipient_data_by_order_id( $order_id ) {
	...
	if (!empty($order_items)) { //if item is empty
		foreach ($order_items as $product_item_key => $product_data) {			
			...
			if( !empty( $product_item_meta ) ){
				$codes = explode(',',$product_item_meta['_woo_vou_codes']);
				foreach ($codes as $key => $code) {
					$code = trim($code);
					$recipient_details[ $code ] = $woo_vou_model->woo_vou_get_recipient_data($product_item_meta);	
				}
			}
		}
	}
...
/**
 * WooCommerce - vouchers price total
 *
 * @package WooCommerce - PDF Vouchers
 * @since 3.7.0
 */
function woo_vou_get_vouchers_price_total( $voucher_codes_data ) {

    $prefix = WOO_VOU_META_PREFIX;
	$data      = isset( $voucher_codes_data['data'] ) ? $voucher_codes_data['data'] : '';   
    $prices    = array();
    if( !empty($data) ){
        foreach ($data as $key => $codes_data) {
            $codes_id           = $codes_data['ID'];
            $code               = get_post_meta( $codes_id, $prefix.'purchased_codes', true );
            $order_id           = get_post_meta( $codes_id, $prefix.'order_id', true );
            $product_info       = woo_vou_display_product_info_data( $order_id, $code );           
            $prices[]           = !empty($product_info['without_currency_price'])?$product_info['without_currency_price']:0;
        }  
    }
...
/**
 * WooCommerce - redeemed vouchers price total
 *
 * @package WooCommerce - PDF Vouchers
 * @since 3.7.0
 */
function woo_vou_get_redeemed_vouchers_price_total( $voucher_codes_data ) {

    $prefix = WOO_VOU_META_PREFIX;
	$data      = isset( $voucher_codes_data['data'] ) ? $voucher_codes_data['data'] : '';   
    $prices    = $redeemed_infos = $main_array = array();
    global $current_user, $woo_vou_model;

    if( !empty($data) ){

        foreach ($data as $keys => $codes_data) {
        	$redeemed_infos = array();
            $voucodeid          = $codes_data['ID'];
            $code               = get_post_meta( $voucodeid, $prefix.'purchased_codes', true );
            $order_id           = get_post_meta( $voucodeid, $prefix.'order_id', true );
            $product_info       = woo_vou_display_product_info_data( $order_id, $code );
            $total_price 		= !empty( $product_info['without_currency_price'] ) ? $product_info['without_currency_price'] : 0;
			...
			if ( !empty($partially_redeemed_data) ) {
				foreach ( $partially_redeemed_data as $key => $value ) {
					...
					$redeemed_infos[] = $redeemed_amount;
				...
				if( !empty( $is_code_used ) && !empty( $redeem_by ) ) {
					$redeemed_infos[] = $total_price;
					
				}
			}

			$main_array[] = $redeemed_infos;
...
function woo_vou_get_expired_vouchers_price_total($voucher_codes_data) {
    $prefix = WOO_VOU_META_PREFIX;
    $data = isset($voucher_codes_data['data']) ? $voucher_codes_data['data'] : '';   
    $prices = $redeemed_infos = $main_array = array();
    global $current_user, $woo_vou_model, $woo_vou_voucher;
    ...
    if (!empty($data)) {
        foreach ($data as $keys => $codes_data) {
            ...
            if (get_post_status($voucodeid)) {
                $voucode = get_post_meta($voucodeid, $prefix . 'purchased_codes', true);
                $order_id = get_post_meta($voucodeid, $prefix . 'order_id', true);
                $product_info = woo_vou_display_product_info_data($order_id, $voucode);
                $total_price = !empty($product_info['without_currency_price']) ? $product_info['without_currency_price'] : 0;
                ...
                if (!empty($order_id)) {
                    ...
                    if ($order && $order->get_id() > 0) {
                        ...
                        // Ensure vou_code_total_price is numeric
                        $vou_code_total_price = is_numeric($vou_code_total_price) ? $vou_code_total_price : 0;

                        $vou_code_total_redeemed_price = $woo_vou_voucher->woo_vou_get_total_redeemed_price_for_vouchercode($voucodeid);
                        $vou_code_total_redeemed_price = is_numeric($vou_code_total_redeemed_price) ? $vou_code_total_redeemed_price : 0;

                        $vou_code_remaining_redeem_price = number_format((float)($vou_code_total_price - $vou_code_total_redeemed_price), 2, '.', '');

                        $main_array[] = $vou_code_remaining_redeem_price;
                    } 
                }
            }
        }
...
/**
 * WooCommerce - append partial date in meta
 *
 * @package WooCommerce - PDF Vouchers
 * @since 3.7.0
 */
function woo_vou_append_partial_redeem_dates( $post_id, $new_date ) {
	$prefix = WOO_VOU_META_PREFIX;
    
    /*// Get the existing meta value
    $current_dates = get_post_meta( $post_id, $prefix .'partial_redeem_dates', true );

    // Check if the current meta value is not empty
    if ( !empty( $current_dates ) ) {
        // Append the new date to the existing dates with a comma separator
        $updated_dates = $current_dates . ',' . $new_date;
    } else {
        // If there's no existing value, set the new date as the first value
        $updated_dates = $new_date;
    }*/

    // Update the post meta with the new value
    //update_post_meta( $post_id, $prefix .'partial_redeem_dates', $updated_dates );
    add_post_meta( $post_id, $prefix .'partial_redeem_dates', $new_date );
    
}

/**
 * WooCommerce - get all date between start and end date
 *
 * @package WooCommerce - PDF Vouchers
 * @since 3.7.0
 */

function woo_vou_get_dates_between($start_date, $end_date) {
    $dates = array();
