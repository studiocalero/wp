...
class WOO_Vou_Upgrade {
	...
	public function woo_vou_upgrade_submenu() {
		...
		$vou_upgrades_screen = add_submenu_page( '', esc_html__( 'PDF Voucher Upgrades', 'woovoucher' ), esc_html__( 'PDF Voucher Upgrades', 'woovoucher' ), 'manage_woocommerce', 'vou-upgrades', array( $this, 'woo_vou_upgrades_screen' ) );
	}
	...
	function woo_vou_trigger_upgrades() {
		...
		if ( !isset($_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'woo_vou_check_security' ) ) {
		    wp_die( esc_html__( 'Nonce verification failed.', 'woovoucher' ), esc_html__( 'Error', 'woovoucher' ), array( 'response' => 403 ) );
		}
		...
		if( !empty( $orders ) ) {//If or der is not empty
			...
			foreach ( $orders as $shop_order ) {//For all old orders
				...
				if( !empty( $order_id ) ) {// If order_id not empty
					...
					//--HPOS--//
					$old_meta 		= $order->get_meta( $prefix . 'order_details', true );
					...
					//--HPOS--//
					$order->delete_meta_data( $prefix . 'order_details' );
				}
			}
		}
	}
		...
		if( !empty( $old_voucher_codes ) ) {//If old voucher codes is not empty
			...
			foreach ( $old_voucher_codes as $voucher_code ) {//For all old voucher codes
				...
				if( !empty( $voucher_code_id ) ) {// If voucher_code_id not empty
					...
					$order_customer_id = $woo_order->get_customer_id();
	...
	public function woo_vou_migrate_voucher_based_purchase_database() {
		...
		if( !empty( $vouchers ) ) {
			foreach ( $vouchers as $voucher) {
				...
				//--HPOS--//
				$woo_order = wc_get_order( $order_id );
				$order_meta_details_arr = $woo_order->get_meta( $prefix . 'meta_order_details', true );
	 ...
	 * @since 4.3.0
     */
    public function woo_vou_admin_notice_for_migrate_voucher_partial_redeem_date_meta() {

    	global $wp;
    	$woo_vou_voucher_partial_redeem_date_on_migration = get_option( 'woo_vou_voucher_partial_redeem_date_on_migration' );
        $woo_vou_plugin_version = get_option( 'woo_vou_plugin_version' );

        if ( empty( $woo_vou_voucher_partial_redeem_date_on_migration ) && !isset( $_GET['woo-vou-upgrade'] ) && version_compare( $woo_vou_plugin_version, '4.9.6', '<=' ) ) {
			$obj_id = get_queried_object_id();
        	$URL = add_query_arg( array(
        		'page' => 'woo-vou-upgrades-voucher',
        		'woo-vou-upgrade' => 'migrate-voucher-partial-redeem-date',
        		'return' => base64_encode( add_query_arg( NULL, NULL ) ),
        	), admin_url('index.php') );
        ...
        // Success message when capabilities get successfully upgraded
		if ( isset( $_GET['woo-vou-migrate-voucher-partial-redeem-date'] ) && $_GET['woo-vou-migrate-voucher-partial-redeem-date'] == 'success' ) {
			// Success Message
			printf('<div class="updated"><p>' . esc_html__( 'WooCommerce Pdf Voucher - Voucher redeem data has been migrated.', 'woovoucher' ) . '</p></div>', 'woovoucher');
		}
    }
	 ...
	 * @since 4.3.0
	 */
	public function woo_vou_admin_run_v496_udater_script() {
		...
		$migrated = get_option( 'woo_vou_voucher_partial_redeem_date_on_migration' );
		...
		$args = array(
			...
			'meta_query'	=> array(
				array(
					'key' => '_woo_vou_partial_redeemed_date_migrated',
					'compare' => 'NOT EXISTS'
				)
			)
		);
		...
		if ( ! empty($vou_code_ids) ) {
			foreach ( $vou_code_ids as $vou_code_id ) {
				...
				$parent_id = wp_get_post_parent_id( $vou_code_id );

				if( !empty( $parent_id ) ){
					$date = get_post_meta( $vou_code_id, $prefix .'used_code_date', true );
					$date = date('Y-m-d', strtotime($date) );	
					add_post_meta( $parent_id, $prefix .'partial_redeem_dates', $date );
					update_post_meta( $vou_code_id, '_woo_vou_partial_redeemed_date_migrated', 'yes' );	
				}
			}
		...
		update_option( 'woo_vou_voucher_partial_redeem_date_on_migration', 'yes' );
		echo 'completed';
		exit;
	}
	...
	public function woo_vou_upgrade_voucher_data_submenu() {
		...
		$vou_upgrades_screen = add_submenu_page( '', esc_html__( 'PDF Voucher Upgrades', 'woovoucher' ), esc_html__( 'PDF Voucher Upgrades', 'woovoucher' ), 'install_plugins', 'woo-vou-upgrades-voucher', array( $this, 'woo_vou_upgrades_voucher_screen' ) );
	}
	...
	public function woo_vou_upgrades_voucher_screen(){
		...
		<div class="wrap">
			...
			<div id="vou-upgrade-status">
				...
				?>
				<input type="hidden" name="" class="woo-vou-upgrade-return" value="<?php echo esc_url($return); ?>" />
			</div>
		</div><?php
	}
	...
	public function woo_vou_plugin_update_action( $upgrader_object, $options ) {
	    if ( $options['action'] == 'update' && $options['type'] == 'plugin' ) {
	        $updated_plugins = $options['plugins'];
	        if ( in_array('woocommerce-pdf-vouchers/woocommerce-pdf-vouchers.php', $updated_plugins ) ) {
	            $license = get_option( 'woo_vou_activated' );
				if( $license ){
		            $woo_vou_license = new WOO_Vou_License();
					$woo_vou_license->woo_vou_verify_license();
				}
	        }
	    }
	}
	...
	public function add_hooks() {
		...
		add_action('admin_notices', array( $this, 'woo_vou_admin_notice_for_migrate_voucher_partial_redeem_date_meta') );

		add_action( 'wp_ajax_woo_vou_migrate_voucher_paerial_redeem_date_meta', array($this, 'woo_vou_admin_run_v496_udater_script') );


		//add submenu page for voucher data upgrade
		add_action( 'admin_menu', array( $this, 'woo_vou_upgrade_voucher_data_submenu' ) );

		add_action('upgrader_process_complete', array( $this, 'woo_vou_plugin_update_action' ), 10, 2);
	}
}