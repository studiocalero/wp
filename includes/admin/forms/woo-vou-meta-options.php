...
wp_nonce_field( WOO_VOU_PLUGIN_BASENAME, 'at_woo_vou_meta_box_nonce' );
	...
	woo_vou_content_begin();
		...
		// voucher font size
		woo_vou_add_number( array( 
			'id' => $prefix . 'pdf_font_size', 
			'class' => 'small-text', 
			'name'=> esc_html__( 'Font Size:', 'woovoucher' ), 
			'default' => 12, 
			'desc' => esc_html__( 'Set the font size for the entire voucher, including the footer.', 'woovoucher' ), 
			'field_desc' => ' px', 
			'min' => 6 
		) );
		
		// voucher font color
		woo_vou_add_color( array( 
			'id' => $prefix . 'pdf_font_color', 
			'name'=> esc_html__( 'Font Colour:', 'woovoucher' ), 
			'default' => '#000000', 
			'desc' => esc_html__( 'Select the font colour for the entire voucher, including the footer.', 'woovoucher' ) 
		) );
