...
//--HPOS--//
$order_customer = $order->get_meta( '_customer_user', true );
...
// Get option whether to hide buyer's information
$vou_hide_buyer_info                         = get_option('vou_hide_buyer_info');
// Get option whether to hide order information
$vou_hide_order_info                         = get_option('vou_hide_order_info');
...
// get partial redeem
$enable_partial_redeem = apply_filters( 
	'woo_vou_enable_partial_redeem_during_check_voucher', 
	woo_vou_check_partial_redeem_by_product_id( $product_id ), 
	$order_id, 
	$voucode 
);
...
$product_info_columns = array(
	...
	'item_price'      => esc_html__( 'Value', 'woovoucher' )
);
// add redeemable price column
$product_info_columns['redeemable_price'] = esc_html__( 'Balance', 'woovoucher' );

// redeem information key parameter
$redeem_info_columns = apply_filters( 
	'woo_vou_check_vou_partial_redeem_info_fields', 
	array(
		'item_name'     => esc_html__( 'Item Name', 'woovoucher' ),
...
$voucher_info_columns = apply_filters( 
...
$buyer_info_columns = apply_filters( 
...
$order_info_columns = apply_filters( 
...
if( $enable_partial_redeem == "yes" || ( !empty( $allow_unlimited_redeem ) && $allow_unlimited_redeem == "yes" ) ) {
	...
	$args                 = array( 'woo_vou_list' => true, 'post_parent' => $voucodeid );
	$partially_redeemed_data = $redeemed_infos = array();
	...
	if( !empty( $partially_redeemed_data ) ) {
		foreach ( $partially_redeemed_data as $key => $value ) {
			...
			$redeemed_infos[$key] = apply_filters( 
				'woo_vou_check_partial_voucher_column_redeemed_info', 
				array(
					"redeem_id"     => $value['ID'],
...
<div class="woo_vou_product_details">
	<?php do_action( 'woo_vou_check_voucher_code_detail_start', $voucodeid, $item_id, $order_id );
	...
	if( !empty( $redeem_info_columns ) && !empty( $redeemed_infos ) && is_array( $redeemed_infos ) ) {
		...
		<h2 class="woo_vou_heading woo_vou_redeem_information">
			<?php echo esc_html__( 'Redeem Information', 'woovoucher' );?>
		</h2>
		
		<div class="woo_pdf_vou_main woo_vou_redeem_info">
			<div class="woo_pdf_pro_tit">
				...
				foreach ( $redeem_info_columns as $col_key => $column ) { ?>
					<div class="<?php echo $product_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						<?php echo $column; ?>
					</div>
				<?php } ?>
			</div>
			...
			foreach( $redeemed_infos as $key => $redeemed_info ) {
				$margin_bottom_0 = ( $key != ( $redeemed_data_cnt - 1 ) ) ? 'woo-vou-margin-bottom-0' : '';
					...
					<div class="<?php echo $product_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						...
						switch ( $col_key ) {
							...
							case 'redeem_by' :
								...
								if ( !empty($redeemed_info['redeem_by']) ) {
						...
						echo apply_filters( 
							'woo_vou_check_partial_voucher_column_value', 
							$column_value, 
							$col_key, 
							$voucodeid, 
							$item_id, 
	...
	if( !empty( $product_info_columns ) ) { //if product info is not empty ?>
		...
		<h2 class="woo_vou_heading woo_vou_product_information">
			<?php echo esc_html__( 'Product Information', 'woovoucher' );?>
		</h2>
	...
	if(!empty($recipient_details)){ 
		...
		<h2 class="woo_vou_heading woo_vou_recipient_information">
			<?php echo esc_html__( 'Recipient Information', 'woovoucher' );?>
		</h2>
		<div class="woo_pdf_vou_main woo_vou_recipient_information">
			<div class="woo_pdf_pro_tit">
				<?php
				...
				foreach( $recipient_data_cols as $recipient_col_key => $recipient_col_val ) {
					if( !empty( $item_meta[$prefix.$recipient_col_key] ) ) {
						echo '<div class="' . $product_col . ' woo_vou_padding">' 
							 . $item_meta[$prefix.$recipient_col_key]['label'] 
							 . '</div>';
					}
				}
				?>
	...
	if( !empty( $voucher_info_columns ) ) { //if voucher info column is not empty ?>
		...
		<h2 class="woo_vou_heading woo_vou_voucher_information">
			<?php echo esc_html__( 'Voucher Information', 'woovoucher' ); ?>
		</h2>
		...
		<div class="woo_pdf_vou_main  woo-vou-voucher-info">
			<div class="woo_pdf_pro_tit">
				...
				foreach ( $voucher_info_columns as $col_key => $column ) { ?>
					<div class="<?php echo $voucher_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						<?php echo $column; ?>
					</div>
				<?php } ?>
			</div>
			<div class="woo_pdf_vou_pro_lst">
				...
				if( $_product ) {
					...
					$allvoucherdata    = apply_filters( 
						'woo_vou_order_voucher_metadata', 
						isset( $allorderdata[$parent_product_id] ) ? $allorderdata[$parent_product_id] : array(), 
						$order_id, 
						$item_id, 
						$parent_product_id 
					);
					...
					if( $_product->is_type('variation') && isset($allvoucherdata) && is_array($allvoucherdata['exp_date']) ){
						if( isset($allvoucherdata['exp_date'][$product_id]) && !empty($allvoucherdata['exp_date'][$product_id]) ){
							$exp_date = !empty( $allvoucherdata['exp_date'][$product_id] ) 
								? $woo_vou_model->woo_vou_get_date_format( $allvoucherdata['exp_date'][$product_id], true ) 
								: esc_html__( 'N/A', 'woovoucher' );
						}
					} elseif( isset($allvoucherdata['exp_date']) && !empty($allvoucherdata['exp_date']) ) {
						$exp_date = !empty( $allvoucherdata['exp_date'] ) 
							? $woo_vou_model->woo_vou_get_date_format( $allvoucherdata['exp_date'], true ) 
							: esc_html__( 'N/A', 'woovoucher' );
					}

					$tmp_exp_date = get_post_meta($voucodeid,$prefix.'exp_date',true);
					$exp_date     = !empty( $tmp_exp_date ) 
						? $woo_vou_model->woo_vou_get_date_format($tmp_exp_date) 
						: esc_html__( 'N/A', 'woovoucher' );
				}
					...
					<div class="<?php echo $voucher_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						...
						switch ( $col_key ) {
							...
							case 'logo' :
								if( !empty( $allvoucherdata['vendor_logo']['src'] ) ) {
									$column_value .= '<div class="woo_pdf_res_vou">' . esc_html__( 'Logo', 'woovoucher') . '</div>'
												  . '<img src="' . esc_url($allvoucherdata['vendor_logo']['src']) . '" alt="" width="70" height="70" />';
								} else {
									$column_value .= '<div class="woo_pdf_res_vou">' . esc_html__( 'Logo', 'woovoucher') . '</div> &nbsp;';
								}
								break;
							...
							case 'voucher_data' :
								...
								<span><?php echo !empty( $allvoucherdata['redeem'] ) ? nl2br( $allvoucherdata['redeem'] ) : esc_html__( 'N/A', 'woovoucher' ); ?></span><br />
								<?php
									...
									foreach ( $allvoucherdata['avail_locations'] as $location ) {
										if( !empty( $location[$prefix.'locations'] ) ) {
											if( !empty( $location[$prefix.'map_link'] ) ) {
												echo '<span><a class="woo-vou-voucher-location-link" target="_blank" href="' 
													 . esc_url($location[$prefix.'map_link']) . '">'
													 . $location[$prefix.'locations'] . '</a></span><br />';
											} else {
								...
								$column_value = 
									'<div class="woo_pdf_res_vou">'. esc_html__( 'Voucher Data', 'woovoucher'). '</div> ' 
								  . '<div class="woo_pdf_val">' . ob_get_clean() . '</div>';
								break;

							case 'expires' :
								$column_value = '<div class="woo_pdf_res_vou">' . esc_html__( 'Expires', 'woovoucher') . '</div>';
								$column_value .= ( isset( $exp_date ) && !empty( $exp_date ) ) 
									? $exp_date 
									: esc_html__( 'Never Expire', 'woovoucher' );
								break;
	...
	if( $vou_hide_buyer_info == 'yes' 
		&& in_array( $user_role, $woo_vou_vendor_role ) 
		&& !in_array( $user_role, $admin_roles ) ) {
		$show_buyer_info = false;
	}

	$show_order_info = true;
	if( $vou_hide_order_info == 'yes' 
		&& in_array( $user_role, $woo_vou_vendor_role ) 
		&& !in_array( $user_role, $admin_roles ) ) {
		$show_order_info = false;
	}
	
	if( $show_buyer_info && !empty( $buyer_info_columns ) ) { //if buyer info is not empty ?>
		
		<h2 class="woo_vou_heading woo_vou_buyer_information">
			<?php echo esc_html__( 'Buyer\'s Information', 'woovoucher' ); ?>
		</h2>
		<div class="woo_pdf_vou_main woo-vou-buyer-info">
			<div class="woo_pdf_vou_tit">
				...
				foreach ( $buyer_info_columns as $col_key => $column ) { ?>
					<div class="<?php echo $buyer_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						<?php echo $column; ?>
					</div>
				<?php } ?>
			</div>
						...
						switch ( $col_key ) { 
							...
							case 'billing_address' :
								if( !empty( $billing_address ) ) {
									$column_value .= '<div class="woo_pdf_res_buyer">' 
													 . esc_html__( 'Billing Address', 'woovoucher') 
													 . '</div><div class="woo_pdf_val">' 
													 . $billing_address 
													 . "</div>";
								} else {
									$column_value .= '<div class="woo_pdf_res_buyer">' 
													 . esc_html__( 'Billing Address', 'woovoucher') 
													 . '</div><div class="woo_pdf_val">&nbsp;</div>';
								}
								break;
							...
							case 'shipping_address' : 
								if(!empty($shipping_address)) {
									$column_value .= '<div class="woo_pdf_res_buyer">' 
													 . esc_html__( 'Shipping Address', 'woovoucher') 
													 . '</div><div class="woo_pdf_val">' 
													 . $shipping_address 
													 . '</div>';
								} else {
	...
	if( $show_order_info && !empty( $order_info_columns ) ) { //if order info is not empty ?>
	
		<h2 class="woo_vou_heading woo_vou_order_information">
			<?php echo esc_html__( 'Order Information', 'woovoucher' );?>
		</h2>
		...
		<div class="woo_pdf_vou_main woo-vou-order-info">
			<div class="woo_pdf_vou_tit">
				...
				foreach ( $order_info_columns as $col_key => $column ) { ?>
					<div class="<?php echo $order_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						<?php echo $column; ?>
					</div>
				<?php } ?>
			</div>
				...
				foreach ( $order_info_columns as $col_key => $column ) { ?>
					<div class="<?php echo $order_col; ?> woo_vou_padding woo_vou_column_<?php echo esc_attr($col_key); ?>">
						...
						switch ( $col_key ) { 
							...
							case 'order_date' :
								$column_value .= '<div class="woo_pdf_res_order">' . esc_html__( 'Order Date', 'woovoucher') . ' </div>'
											   . $woo_vou_model->woo_vou_get_date_format( $order_date, true );
								break;
							
							case 'payment_method' : 
								$column_value .= '<div class="woo_pdf_res_order">' . esc_html__( 'Payment Method', 'woovoucher') . '</div>'
											   . $payment_method;
								break;
							
							case 'order_total':
								$column_value .= '<div class="woo_pdf_res_order">' . esc_html__( 'Order Total', 'woovoucher') . '</div>'
											   . wc_price($woo_vou_wc_currency_switch->woo_vou_multi_currency_price($order->get_total(), $order_id)); 
								break;
							
							case 'order_discount' : 
								$column_value .= '<div class="woo_pdf_res_order">' . esc_html__( 'Order Discount', 'woovoucher') . '</div>'
											   . wc_price(
												   $woo_vou_wc_currency_switch->woo_vou_multi_currency_price( 
													   $order->get_total_discount(), 
													   $order_id 
												   ), 
												   array('currency' => woo_vou_get_order_currency($order))
											   );
								break;
	...
	$voucher_data     = get_post( $voucodeid ); // Get voucher data
	$vou_used_codes   = get_post_meta( $voucodeid, $prefix.'used_codes', true ); // Check if voucher is used

	/*
	* If user role is admin
	* If user role is voucher vendor then check the user is voucher post author 
	* OR checks whether "Enable Vendor to access all voucher codes" is ticked
	* If user is the customer of this voucher's order and "Check and Redeem" are both allowed for logged users
	*/
	...
	if( in_array( $user_role, $admin_roles ) ) {
		...
		if ( ( $voucher_data->post_author == $current_user->ID ) 
			 || ( $vou_enable_vendor_access_all_voucodes == 'yes' ) 
			 || ( $vou_allow_secondary_vendor_redeem_primary_voucher == 'yes' && $voucher_data->post_author != $current_user->ID  ) ) {
			$allow_user_redeem = true;
		}
	} elseif( $order_customer == $current_user->ID ) {
		if( ( $vou_enable_logged_user_check_voucher_code == 'yes' ) 
			&& ( $vou_enable_logged_user_redeem_vou_code == 'yes' ) ) {
			$allow_user_redeem = true;
		}
	}
	 ...
	 * If partial redeem enabled then show partial redeem option
	 * If condition is satisfied then user can see redeem button
	 */
	...
	?>
		...
		<div class="woo_pdf_vou_main">
			<h2 class="woo_vou_heading woo_vou_redeem_options">
				<?php esc_html_e( 'Redeem Options', 'woovoucher' ); ?>
			</h2>
			<div class=" woo-vou-margin-bottom-0">
				...
				<div class="col-2 woo_vou_padding">
					...
					if( !empty( $partially_redeemed ) && $partially_redeemed == 'partial' ) { ?>
						<span class="description">
							...
							echo sprintf( 
								esc_html__( 
									'If you select %sFull%s method then it will redeem remaining amount. If you select %sPartial%s then you have option to enter the partial redeem amount.', 
									'woovoucher' 
								), 
								'<b>', 
								'</b>', 
					...
					<?php } else { ?>
						<span class="description">
							...
							echo sprintf( 
								esc_html__( 
									'If you select %1$sFull%1$s method then it will redeem full amount. If you select %2$sPartial%2$s then you have option to enter the partial redeem amount.', 
									'woovoucher' 
								), 
								'<b>', 
								'</b>' 
			...
			<div class="woo-vou-partial-redeem-amount woo-vou-margin-bottom-0">
				<div class="col-6 woo_vou_padding">
					<label for="vou_partial_redeem_amount"><?php esc_html_e( 'Redeem Amount', 'woovoucher' ); ?></label>
				</div>
				<div class="col-2 woo_vou_padding">
					<input type="number" name="vou_partial_redeem_amount" id="vou_partial_redeem_amount" 
						   value="<?php echo $vou_code_remaining_redeem_price; ?>" 
						   max="<?php echo $vou_code_remaining_redeem_price; ?>" step="any"/><br />
					<span class="description">
						<?php esc_html_e( 'Enter the amount you want to redeem.', 'woovoucher' ); ?>
					</span>
