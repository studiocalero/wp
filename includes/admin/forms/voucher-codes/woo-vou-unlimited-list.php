...
class WOO_Vou_Unlimited_Redeemed_List extends WP_List_Table {
	...
	function __construct(){
		...
		$this->per_page	= apply_filters( 'woo_vou_unlimited_redeemed_posts_per_page', 10 ); // Per page		
	}
	...
	function column_default( $item, $column_name ) {
		...
		return apply_filters( 'woo_vou_unlimited_redeemed_column_value', $column_value, $column_name, $item );
	}
	...
	function get_bulk_actions() {
		...
		$actions    = apply_filters( 'woo_vou_unlimited_redeemed_list_bulk_actions', $actions );
    ...
    public function column_code_details( $item ){
    	...
    	if( !get_post_status( $item['post_name'] ) ) {
	        ...
	        $column_value = sprintf( '<a class="woo_vou_delete_voucher_code woo-vou-action-button" href="?page=%s&vou-data=%s&action=%s&code_id=%s&order_id=%s" title="'.esc_html__('Delete', 'woovoucher').'"></a>', esc_attr( $_REQUEST['page'] ) , 'purchased', 'delete', esc_attr( $item['ID'] ), esc_attr( $item['post_name'] ) );
		...
		$column_value = apply_filters( 'woo_vou_unlimited_redeemed_code_details_column_value', $column_value, $item );
...
$WooUnlimitedRedeemedListTable = new WOO_Vou_Unlimited_Redeemed_List();

//Fetch, prepare, sort, and filter our data...
$WooUnlimitedRedeemedListTable->prepare_items();
?>
...
<div class="wrap"><?php 
	...
	$WooUnlimitedRedeemedListTable->views();?>

    <?php do_action( 'woo_vou_purchased_list_before_form', $WooUnlimitedRedeemedListTable ); ?>
    ...
    <form id="product-filter" method="GET" action="">
        ...
        <input type="hidden" name="vou-data" value="<?php echo esc_attr( $_REQUEST['vou-data'] ); ?>" />
        <!-- Search Title -->
        <?php $WooUnlimitedRedeemedListTable->search_box( esc_html__( 'Search', 'woovoucher' ), 'woovoucher' ); ?>
        ...
        <?php $WooUnlimitedRedeemedListTable->display(); ?>
    </form>
     <?php do_action( 'woo_vou_purchased_list_after_form', $WooUnlimitedRedeemedListTable ); ?>
</div>
<?php 
	// Check the Enable Change Voucher Expiry Date 
	$vou_change_expiry_date = get_option( 'vou_change_expiry_date' );
