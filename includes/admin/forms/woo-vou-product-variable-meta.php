...
This setting modifies the global voucher expiration date setting and overrides voucher\'s expiration date value. Set expiration date type "Default" to use the global/voucher settings. ', 'woovoucher' ); ?>" class="woocommerce-help-tip"></span>
	...
	<?php
	echo '<p class="woo-vou-variations-title">'.esc_html__( 'Gift Voucher', 'woovoucher' ).'</p>';
	echo '<p>'.esc_html__( 'Please select the fields which you wanted to hide for this variation, Leave it empty to use product level settings.', 'woovoucher' ).'</p>';
	...
	$woo_voo_product_id = wp_get_post_parent_id($variation_id);	
	$count = 1;
	foreach ( $woo_vou_recipient_details_columns as $recipient_key => $recipient_val ) {
		$woo_voo_enabled_recipient_field = get_post_meta( $woo_voo_product_id, $prefix.'enable_'.$recipient_key, true );

		if( $woo_voo_enabled_recipient_field == 'yes' ){

			$class = ( $count % 2 ) ? 'form-row form-row-first' : 'form-row form-row-last';

			woocommerce_wp_checkbox(
				array(
					'id'            => $prefix.'hide_'.$recipient_key.'[' . $loop . ']',
					'label'         => $recipient_val['label'],
					'wrapper_class' => $class,
					'desc_tip'      => '',
					'description'   => '',
					'class'			=> 'woo_vou_hide_recipient_fields checkbox',
					'value'         => get_post_meta( $variation_id, $prefix.'hide_'.$recipient_key, true )
				)
			);
