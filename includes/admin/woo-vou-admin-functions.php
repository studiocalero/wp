...
function woo_vou_product_save_data($post_id, $post) {
    ...
    // get force override template
    $woo_vou_force_product_template_use = !empty($_POST[$prefix . 'force_product_template_use']) ? 'yes' : '';
    ...
    // PDF Template
    if( !empty( $woo_vou_force_product_template_use ) ) {
        update_post_meta($post_id, $prefix . 'force_product_template_use', $woo_vou_force_product_template_use);
    } else{
        delete_post_meta($post_id, $prefix . 'force_product_template_use' );
    }
    ...
    /**
     * Pay your price start
     */
    
    $product  = wc_get_product($post_id);
    $is_variable = (is_object($product) && ($product->is_type('variable') || $product->is_type('variation'))) ? 1 : 0;

    //Enabel pay your price only for the simple product
    $woo_vou_enabel_pay_your_price = !empty($_POST[$prefix . 'enabel_pay_your_price']) ? 'yes' : '';
    if( !empty( $woo_vou_enabel_pay_your_price ) && !$is_variable ) {
        update_post_meta($post_id, $prefix . 'enabel_pay_your_price', $woo_vou_enabel_pay_your_price);
    } else{
        delete_post_meta($post_id, $prefix . 'enabel_pay_your_price' );
    }

    // Save pay your minimum price
	$woo_vou_pay_your_min_price = isset($_POST[$prefix . 'woo_vou_pay_your_min_price']) && !empty($_POST[$prefix . 'woo_vou_pay_your_min_price']) ? $_POST[$prefix . 'woo_vou_pay_your_min_price'] : '';

    if ( !empty( $woo_vou_pay_your_min_price ) ) {
	   update_post_meta($post_id, $prefix . 'woo_vou_pay_your_min_price', $woo_vou_pay_your_min_price);
    } else {
        delete_post_meta($post_id, $prefix . 'woo_vou_pay_your_min_price');
    }

    // Save pay your maximum price
	$woo_vou_pay_your_max_price = isset($_POST[$prefix . 'woo_vou_pay_your_max_price']) && !empty($_POST[$prefix . 'woo_vou_pay_your_max_price']) ? $_POST[$prefix . 'woo_vou_pay_your_max_price'] : '';

    if ( !empty( $woo_vou_pay_your_max_price ) ) {
	   update_post_meta($post_id, $prefix . 'woo_vou_pay_your_max_price', $woo_vou_pay_your_max_price);
    } else {
        delete_post_meta($post_id, $prefix . 'woo_vou_pay_your_max_price');
    }

    // Save pay your recommended price
	$woo_vou_pay_your_recommended_price = isset($_POST[$prefix . 'woo_vou_pay_your_recommended_price']) && !empty($_POST[$prefix . 'woo_vou_pay_your_recommended_price']) ? $_POST[$prefix . 'woo_vou_pay_your_recommended_price'] : '';

    if ( !empty( $woo_vou_pay_your_recommended_price ) ) {
	   update_post_meta($post_id, $prefix . 'woo_vou_pay_your_recommended_price', $woo_vou_pay_your_recommended_price);
    } else {
        delete_post_meta($post_id, $prefix . 'woo_vou_pay_your_recommended_price');
    }

    //Save hide minimum pay your price
    $woo_vou_woo_vou_hide_pay_your_min_price = !empty($_POST[$prefix . 'woo_vou_hide_pay_your_min_price']) ? 'yes' : '';
    if( !empty( $woo_vou_woo_vou_hide_pay_your_min_price ) ) {
        update_post_meta($post_id, $prefix . 'woo_vou_hide_pay_your_min_price', $woo_vou_woo_vou_hide_pay_your_min_price);
    } else{
        delete_post_meta($post_id, $prefix . 'woo_vou_hide_pay_your_min_price' );
    }

    //Save hide maximum pay your price
    $woo_vou_woo_vou_hide_pay_your_max_price = !empty($_POST[$prefix . 'woo_vou_hide_pay_your_max_price']) ? 'yes' : '';
    if( !empty( $woo_vou_woo_vou_hide_pay_your_max_price ) ) {
        update_post_meta($post_id, $prefix . 'woo_vou_hide_pay_your_max_price', $woo_vou_woo_vou_hide_pay_your_max_price);
    } else{
        delete_post_meta($post_id, $prefix . 'woo_vou_hide_pay_your_max_price' );
    }

    // Save pay your price input field type
	$woo_vou_pay_your_price_input_field_type = isset($_POST[$prefix . 'woo_vou_pay_your_price_input_field_type']) && !empty($_POST[$prefix . 'woo_vou_pay_your_price_input_field_type']) ? $_POST[$prefix . 'woo_vou_pay_your_price_input_field_type'] : '';

    if ( !empty( $woo_vou_pay_your_price_input_field_type ) ) {
	   update_post_meta($post_id, $prefix . 'woo_vou_pay_your_price_input_field_type', $woo_vou_pay_your_price_input_field_type);
    } else {
        delete_post_meta($post_id, $prefix . 'woo_vou_pay_your_price_input_field_type');
    }

    // Save pay your price input field type amount
	$woo_vou_pay_your_price_input_field_type_amount_values = isset($_POST[$prefix . 'woo_vou_pay_your_price_input_field_type_amount_values']) && !empty($_POST[$prefix . 'woo_vou_pay_your_price_input_field_type_amount_values']) ? $_POST[$prefix . 'woo_vou_pay_your_price_input_field_type_amount_values'] : '';

    if ( !empty( $woo_vou_pay_your_price_input_field_type_amount_values ) ) {
	   update_post_meta($post_id, $prefix . 'woo_vou_pay_your_price_input_field_type_amount_values', $woo_vou_pay_your_price_input_field_type_amount_values);
    } else {
        delete_post_meta($post_id, $prefix . 'woo_vou_pay_your_price_input_field_type_amount_values');
    }
}
 ...
 * @since 1.1.0
 */
function woo_vou_display_voucher_data( $post_or_order_object ) {
  //--HPOS--//  
...
function woo_vou_product_save_variable_meta($variation_id, $i) {
    ...
    if (!empty($variation_id)) {
        ...
        $woo_vou_recipient_details_columns = woo_vou_voucher_recipient_details();
        $woo_vou_recipient_details_columns = array_merge( $woo_vou_recipient_details_columns, array(
            'recipient_delivery_method' => array( 
                'label' =>  esc_html__( 'Delivery Method', 'woovoucher' ),
                'type'	=> 'text'
            )
        ));
        foreach ( $woo_vou_recipient_details_columns as $recipient_key => $recipient_val ) {
            $variable_voucher_hide_recipient = isset($_POST[$prefix.'hide_'.$recipient_key][$i]) ? $_POST[$prefix.'hide_'.$recipient_key][$i]: '';
            update_post_meta( $variation_id, $prefix.'hide_'.$recipient_key, $variable_voucher_hide_recipient ); 
        }
...
function woo_vou_voucher_redeem_popup(){
    ...
    if( !empty( $voucodeid ) ) {		
        ...
        if( !empty($order_id) && !empty($cart_details) ){
            ...
            $response = apply_filters('woo_vou_voucher_code_popup_check_last_response', $response, $voucodeid );
 ...
 * @since 7.4.4
 */
function woo_vou_voucher_recipient_add_details_func( $recipient_array ) {

    $vou_gift_voucher_custom_fields = get_option( 'vou_gift_voucher_custom_fields' );

    // If not empty then display fields in voucher section
    if( !empty( $vou_gift_voucher_custom_fields ) ){

        foreach ( $vou_gift_voucher_custom_fields as $field ) {
            $recipient_array[ $field['id'] ] = array(
                'label' => $field['name'],
                'type'  => $field['type'],
            );
        }
...
/**
 * Code to add subfields like Label and so on
 *
 * @package WooCommerce - PDF Vouchers
 * @since 7.4.4
 */
function woo_vou_voucher_recipient_field( $fields ) {

    global $woo_vou_admin_meta;
    $prefix                = WOO_VOU_META_PREFIX;
    $vou_codes_error_class = ' woo-vou-display-none ';

    // Get saved custom field data
    $vou_gift_voucher_custom_fields = get_option( 'vou_gift_voucher_custom_fields' );
    ...
    if( !empty( $vou_gift_voucher_custom_fields ) ){
        ...
        foreach ( $vou_gift_voucher_custom_fields as $field ) {
            ...
            if ( $prefix . 'enable_' . $field['id'] == $fields['id'] ) {

                $woo_vou_admin_meta->woo_vou_add_checkbox(
                    array(
                        'id'          => $prefix . 'enable_' . $field['id'],
                        'field_type'  => 'recipient',
                        'label'       => sprintf( esc_html__( 'Enable %s:' , 'woovoucher' ), $field['name'] ),
                        'description' => sprintf( esc_html__( 'To enable the %s on the product page.', 'woovoucher' ), $field['name'] ),
                    )
                );
                echo '<div class="recipient-detail-wrap">';
                $woo_vou_admin_meta->woo_vou_add_text(
                    array(
                        'id'          => $prefix . $field['id'] . '_label',
                        'class'       => 'woo_vou_recipient_text',
                        'wrap_class'  => 'woo_vou_recipient_details',
                        'label'       => esc_html__( 'Label:', 'woovoucher' ),
                        'description' => esc_html__( '', 'woovoucher' ),
                        'placeholder' => esc_html__( 'Name', 'woovoucher' ),
                    )
                );
                $woo_vou_admin_meta->woo_vou_add_text(
                    array(
                        'id'          => $prefix . $field['id'] . '_max_length',
                        'class'       => 'woo_vou_recipient_text',
                        'wrap_class'  => 'woo_vou_recipient_details',
                        'label'       => esc_html__( 'Max Length:', 'woovoucher' ),
                        'description' => esc_html__( '', 'woovoucher' ),
                    )
                );
                $woo_vou_admin_meta->woo_vou_add_cust_checkbox(
                    array(
                        'id'          => $prefix . $field['id'] . '_is_required',
                        'label'       => esc_html__( 'Required:', 'woovoucher' ),
                        'description' => esc_html__( 'Make this field required in order to add a voucher product to the cart', 'woovoucher' ),
                    )
                );
                $woo_vou_admin_meta->woo_vou_add_textarea_input(
                    array(
                        'id'          => $prefix . $field['id'] . '_desc',
                        'field_type'  => 'recipient',
                        'label'       => esc_html__( 'Description:', 'woovoucher' ),
                        'description' => esc_html__( 'Enter the description which you want to show on product page.', 'woovoucher' ),
                        'label_style' => 'display: block; float: none; width: auto !important;',
                        'rows'        => 2,
                        'cols'        => '5',
                    )
                );
...
/**
 * Replace shortcode with dynamic values
 *
 * @package WooCommerce - PDF Vouchers
 * @since 7.4.4
 */
function woo_vou_pdf_template_add_shortcodes( $voucher_template_html, $orderid, $item_key, $items, $voucodes, $productid ) {
    ...
    // If not empty then replace value
    if( !empty( $vou_gift_voucher_custom_fields ) ){
        foreach ( $vou_gift_voucher_custom_fields as $field ) {

            // Get recipient from name
            $recipient_from_name = $order_item->get_meta( $prefix . $field['id'] );
            ...
            if ( $recipient_from_name ) {
                ...
                $voucher_template_html = str_replace( '{' . $field['id'] . '}', $recipient_from_name_value, $voucher_template_html );
            }
        }
    }
