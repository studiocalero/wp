...
class Woo_Vou_Redeem_Notification extends WC_Email {
	...
	public function trigger( $vou_redeem_info ) {
        ...
        $send_email_to_customer_choosen_recipient = $this->get_option( 'enabled_customer_choosen_recipient_email_notification' );
        $send_email_to_vendor = $this->get_option( 'enabled_vendor_email_notification' );

        if( $send_email_to_customer == 'yes' || $send_email_to_customer_choosen_recipient == 'yes' || $send_email_to_vendor == 'yes'){
        	...
        	$customer_choosen_recipient_email = isset($all_recipients['customer_choosen_recipient_email'])?implode(',',$all_recipients['customer_choosen_recipient_email']):'';
    		...
    		if( !empty( $customer_choosen_recipient_email ) ) {
    			$this->send( $customer_choosen_recipient_email, $this->get_subject(), $this->get_content(), $headers, $this->get_attachments() );	
    		}
        }
        else{
	...
	public function get_recipient_data() {
		...
		if( !empty( $send_email_to_customer_choosen_recipient ) && $send_email_to_customer_choosen_recipient == 'yes' && !empty( $this->object['order_id'] ) ) {

			// Get order recipient data
			$customer_choosen_recipient = woo_vou_get_recipient_data_by_order_id( $this->object['order_id'] );
			if( !empty( $customer_choosen_recipient ) ){
				$recipient = !empty( $customer_choosen_recipient[ $this->object['voucode'] ] ) ? $customer_choosen_recipient[ $this->object['voucode'] ] : '';
				if( !empty( $recipient['recipient_email'] ) ){
					$recipient_email_choosen[] = $recipient['recipient_email'];
					$recipient_email_arr['customer_choosen_recipient_email'] = $recipient_email_choosen;
				}
			}	
		}
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'enabled_customer_choosen_recipient_email_notification' => array(
				'title'   		=> esc_html__( 'Enable/Disable Recipient Email Notification', 'woovoucher' ),
				'type'    		=> 'checkbox',
				'label'   		=> esc_html__( 'Enable/Disable Recipient Email Notification', 'woovoucher' ),
				'default' 		=> 'no',
				'description'	=> esc_html__( 'Check this box if you want to send Voucher Redeem Notification Email to customer choosen recipient(s).' )
			),
