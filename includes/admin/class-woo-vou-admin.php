...
class WOO_Vou_Admin {
    ...
    /**
     * Handles to bulk delete voucher listing
     * 
     * @package WooCommerce - PDF Vouchers
     * @since 1.0.0
     */
    public function woo_vou_voucher_bulk_delete() {

        if( ( ( isset( $_GET['action'] ) && $_GET['action'] == 'delete' ) || ( isset( $_GET['action2'] ) && $_GET['action2'] == 'delete' )  ) && isset($_GET['page']) && $_GET['page'] == 'woo-vou-codes' && ( !empty( $_GET['purchasedvou'] ) || !empty( $_GET['usedvou'] )  ) ) { 

            $tab = !empty($_GET['vou-data'])?$_GET['vou-data']:'purchased';
            ...
            if( $tab == 'purchased' ){
                ...
                $plural = 'purchasedvous';
            }elseif( $tab == 'unlimited' ){
                $plural = 'usedvous';
            }
            
            if (isset($_REQUEST['_wpnonce']) && !wp_verify_nonce($_REQUEST['_wpnonce'], 'bulk-'.$plural)) {
                wp_die( __( 'Security check failed', 'woovoucher' ) );
            }

            //get bulk product array from $_GET
            if( !empty($_GET['purchasedvou']) ){
                $action_on_ids = $_GET['purchasedvou'];
            } elseif( !empty($_GET['usedvou']) ){
                $action_on_ids = $_GET['usedvou'];
            }else{
                $action_on_ids = array();
            }
            
            $redirect_url = add_query_arg( array( 'page' => 'woo-vou-codes','vou-data'=> $tab ), admin_url( 'admin.php' ) );        
            ...
            if( count( $action_on_ids ) > 0 ) { //check there is some checkboxes are checked or not 
                ...
                foreach ( $action_on_ids as $purchasedvou_id ) {
                    
                    //parameters for delete function
                    $args = array (
                                    'purchasedvou_id' => $purchasedvou_id
                                  );
                                
                    //call delete function from model class to delete records
                    $this->model->woo_vou_bulk_delete( $args );
                }
    ...
    public function woo_vou_order_meta_boxes() {
        ...
        //--HPOS--//
        if ( OrderUtil::custom_orders_table_usage_is_enabled() && !empty($_GET['page']) && $_GET['page'] == 'wc-orders' ) {
        ...
        if( !empty( $woo_order) ){
            $order_details     = $woo_order->get_meta( $prefix . 'meta_order_details', true );
            $order_hide_vou    = $woo_order->get_meta( $prefix . 'order_hide_voucher_data', true );    
        }
        
        $order_hide_vou = apply_filters('order_hide_voucher_data', $order_hide_vou, $order_id );
        
        //Check if voucher order & not setup as hide data
        if( $order_details && ! $order_hide_vou ) {
            add_meta_box('woo-vou-order-voucher-details', esc_html__('Voucher Details', 'woovoucher'), 'woo_vou_display_voucher_data', $screen, 'normal', 'default');
        }
    }
    ...
    public function woo_vou_send_gift_notification_email() {
        ...
        $args = array(
            ...
            'return'    => 'ids',
            'meta_key'  => $prefix . 'recipient_email_sent',
            'meta_value'=> '',
            'meta_compare'  => 'NOT EXISTS', 
        );

        $woo_vou_get_all_orders = wc_get_orders( $args );
        ...
        foreach ($woo_vou_get_all_orders as $order_id) {
            ...
            if (!empty($order_items)) { //if item is empty
                ...
                if ($recipient_gift_email_send) {
                    ...
                    $order->update_meta_data( $prefix . 'recipient_email_sent', true );
                    $order->save();
                }
    ...
    public function woo_vou_generate_system_log() {
        ...
        if (!empty($_GET['tab']) && $_GET['tab'] == 'woo-vou-settings' && !empty($_GET['woo_vou_gen_sys_log']) && $_GET['woo_vou_gen_sys_log'] == 1) {
            ...
            if ( !isset($_GET['security'] ) || !wp_verify_nonce( $_GET['security'], 'woo_vou_check_security') ) {
                 wp_die(
                    __('Nonce verification failed.', 'woovoucher'), 
                    __('Error', 'woovoucher'), 
                    array('response' => 403)
                );
            }
    ...
    public function woo_vou_codes_page_voucher_information_update() {
        ...
        if (isset($_REQUEST['woo_vou_voucher_information_update']) && isset($_REQUEST['woo_vou_order_id'])) {
            ...
            //--HPOS--//
            $woo_order = wc_get_order( $woo_vou_order_id );
    ...
    public function woo_vou_add_dashboard_widgets() {
       ...
       // wp_add_dashboard_widget('dashboard_widget', esc_html__('WooCommerce PDF Vouchers Status', 'woovoucher'), array($this, 'woo_vou_dashboard_widget_function'));
        //wp_add_dashboard_widget('dashboard_widget', esc_html__('WooCommerce PDF Vouchers Last 1 Year Status', 'woovoucher'), array($this, 'woo_vou_dashboard_widget_function'));


        wp_add_dashboard_widget(
            'dashboard_widget_voucher_overview', 
            '<span class="dashicons dashicons-chart-pie"></span> ' . esc_html__('Vouchers Overview', 'woovoucher'), 
            array($this, 'woo_vou_dashboard_widget_function')
        );
    ...
    public function woo_vou_dashboard_widget_function( $post, $callback_args ) {
        ...
        wp_enqueue_script( 'woo-vou-dashboard-script' );

        $months = array(
                '01' => __('January', 'woovoucher'),
                '02' => __('February', 'woovoucher'),
                '03' => __('March', 'woovoucher'),
                '04' => __('April', 'woovoucher'),
                '05' => __('May', 'woovoucher'),
                '06' => __('June', 'woovoucher'),
                '07' => __('July', 'woovoucher'),
                '08' => __('August', 'woovoucher'),
                '09' => __('September', 'woovoucher'),
                '10' => __('October', 'woovoucher'),
                '11' => __('November', 'woovoucher'),
                '12' => __('December', 'woovoucher'),
            );
        ...
        // Your HTML and output remains the same
        $purchased_codes_total = 0;
        $purchased_codes_price_total = 0;
        $unredeemed_codes_data_price_total = 0;
        $used_codes_total = 0;
        $used_codes_data_price_total = 0;
        $expired_codes_total = 0;
        $expired_codes_data_price_total = 0;

        ?>

        <div class=" woo-vou-dash-clear-btn">
            <select class="woo_vou_filter_month">
                <option value=""><?php _e('Select Month', 'woovoucher'); ?></option>
                <?php foreach($months as $key => $month): ?>
                    <option value="<?php echo esc_attr($key); ?>"><?php echo esc_html($month); ?></option>
                <?php endforeach; ?>
            </select>

            <select class="woo_vou_filter_year">
                <?php foreach($years as $year): ?>
                    <option value="<?php echo esc_attr($year); ?>"><?php echo esc_html($year); ?></option>
                <?php endforeach; ?>
            </select>

            <button class="button button-primary woo_vou_filter_go"><?php _e('Go', 'woovoucher'); ?></button>
        </div>
        <div class="dash-clear-table woo_vou_filter_ajax_response">
            <div class="woo-vou-loading-spinner" style="display: none;"></div>
            <table class="woo-vou-dash-voucher-status-table">
            <thead>
                <tr>
                    <th><?php _e( 'Vouchers Status', 'woovoucher' ); ?></th>
                    <th><?php _e( 'Vouchers', 'woovoucher' ); ?></th>
                    <th><?php _e( 'Total Amount', 'woovoucher' ); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php if ( isset( $purchased_codes_total ) ) { ?>
                <tr>
                    <td><?php _e( 'Purchased Vouchers', 'woovoucher' ); ?></td>
                    <td><?php echo $purchased_codes_total; ?></td>
                    <td class="woo-vou-dash-amount" style="color: blue;"><?php echo strip_tags( wc_price( $purchased_codes_price_total ) ); ?></td>
                </tr>
                <?php } ?>
                <?php if ( isset( $purchased_codes_total ) ) { ?>
                <tr>
                    <td><?php _e( 'Unredeemed Vouchers', 'woovoucher' ); ?></td>
                    <td><?php echo $purchased_codes_total; ?></td>
                    <td class="woo-vou-dash-amount" style="color: #ff8b00;"><?php echo strip_tags( wc_price( $unredeemed_codes_data_price_total ) ); ?></td>
                </tr>
                <?php } ?>
                <?php if ( isset( $used_codes_total ) ) { ?>
                <tr>
                    <td><?php _e( 'Redeemed Vouchers', 'woovoucher' ); ?></td>
                    <td><?php echo $used_codes_total; ?></td>
                    <td class="woo-vou-dash-amount" style="color: green;"><?php echo strip_tags( wc_price( $used_codes_data_price_total ) ); ?></td>
                </tr>
                <?php } ?>
                <?php if ( isset( $expired_codes_total ) ) { ?>
                <tr>
                    <td><?php _e( 'Expired Vouchers', 'woovoucher' ); ?></td>
                    <td><?php echo $expired_codes_total; ?></td>
                    <td class="woo-vou-dash-amount" style="color: red;"><?php echo strip_tags( wc_price( $expired_codes_data_price_total ) ); ?></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
     ...
     * @since 3.5.0
     */
    public function woo_vou_dashboard_widget_ajax_function() {
        ...
        if ($month) {
            $date_after = $year . '-' . $month . '-01';
            $date_before = date('Y-m-t', strtotime($date_after));
        }

        // Updated date_query to reflect selected month/year
        $date_query = array(
            array(
                'after'     => $date_after,
                'before'    => $date_before,
        ...
        // Get option whether to allow all vendors to redeem voucher codes
        ...
        // Get purchased codes data without caching
        ...
        $purchased_codes_price_total = woo_vou_get_vouchers_price_total( $purchased_codes_data );

        $unredeemed_codes_data_price_total = woo_vou_get_expired_vouchers_price_total( $purchased_codes_data );

        $purchased_codes_total = isset( $purchased_codes_data['total'] ) ? $purchased_codes_data['total'] : '';

        // Get used codes data without caching
        ...
        $used_codes_data_price_total = woo_vou_get_redeemed_vouchers_price_total( $used_codes_data );

        // Get expired codes data without caching
        ...
        $expired_codes_data_price_total = woo_vou_get_expired_vouchers_price_total( $expired_codes_data );
        ...
        <?php
        $output = ob_get_clean();
        wp_send_json_success(array('html' => $output)); // Send the HTML response
    ...
    public function woo_vou_check_order_admin_notice(){
        ...
        if( !empty($order_id) ){
            ...
            if( !empty( $post_type ) && $post_type == 'shop_order') {
                ...
                //--HPOS--//
                $is_pdf_voucher = $order->get_meta( $prefix . 'multiple_pdf', true );
                $is_cancel_to_complete = $order->get_meta( $prefix . 'order_hide_voucher_data', true );
    ...
    */
    ...
    public function woo_vou_dismiss_order_cancelled(){
     ...
     /**
     * Handle to set QR image in user profile page
     *
     * @package WooCommerce - PDF Vouchers
     * @since 4.6.0
    */
   public function woo_vou_vendor_login_qrcode_image($user) {

        if (!in_array(WOO_VOU_VENDOR_ROLE, $user->roles)) {
            return false;
        }

        $token = get_user_meta( $user->ID, '_woo_vou_vendor_token', true );

        if ( !$token ) {
            $token = bin2hex( random_bytes( 16 ) );
            update_user_meta( $user->ID, '_woo_vou_vendor_token', $token );
        }
        ...
        $qr_data_encrypted = woo_vou_encrypt( json_encode( $qr_data ) );

        $qr_url = add_query_arg( array(
            'woo_vou_vendor_qrlogin_code' => urlencode( $qr_data_encrypted ),
        ), site_url() );
        ?>

        <h3 class="heading"><?php esc_html__('QR Code.', 'woovoucher'); ?></h3>
        <table class="form-table">
            <tr>
                <th><label for="contact"><?php echo esc_html__('Your Login QR Code', 'woovoucher'); ?></label></th>
                <td>
                    


                    <img width="500" height="500" class="woo-vou-download-qr-img" src="<?php echo esc_url($qr_url); ?>"><br>
                    <a download="your-qr-code" href="<?php echo esc_url($qr_url); ?>" class="button woo-vou-download-qr-btn" target="_blank"><?php echo esc_html__('Download QR code', 'woovoucher'); ?></a>
                    <p class="description"><?php echo esc_html__('Use this QR code to quickly log into your account upon scan, without entering username and password.', 'woovoucher'); ?></p>
                </td>
            </tr>
        </table>
        <?php
     ...
     * Adding Hooks
     *
     * @package WooCommerce - Remove woocommerce menu for vandor
     * @since 1.0.0
     */
    public function woo_vou_remove_woo_menu_for_vandor() {
    ...
    public function add_hooks() {
        ...
        //ajax call to share shortcode details
        add_action('wp_ajax_woo_vou_shortcode_detail', array($this->render, 'woo_vou_shortcode_detail'));
        add_action('wp_ajax_nopriv_woo_vou_shortcode_detail', array($this->render, 'woo_vou_shortcode_detail'));
        ...
        add_action( 'edit_user_profile', array($this, 'woo_vou_vendor_login_qrcode_image') );
        add_action( 'show_user_profile', array($this, 'woo_vou_vendor_login_qrcode_image') );
       
        add_filter( 'woo_vou_voucher_recipient_details', 'woo_vou_voucher_recipient_add_details_func' );

        // Add action to adding custom fields section into recipient fields
        add_action( 'woo_vou_custom_recipient_field', 'woo_vou_voucher_recipient_field');

        // Add filter to add voucher template shortcodes in download pdf
        add_filter('woo_vou_pdf_template_inner_html', 'woo_vou_pdf_template_add_shortcodes', 10, 6 );

        //add admin init for bulk delete functionality
        add_action( 'admin_init' , array($this,'woo_vou_voucher_bulk_delete'), 1);

        add_action( 'admin_menu' , array($this,'woo_vou_remove_woo_menu_for_vandor'), 999);


        add_action('wp_ajax_woo_vou_dashboard_widget', array($this, 'woo_vou_dashboard_widget_ajax_function'));
        //add_action('wp_ajax_nopriv_woo_vou_dashboard_widget', array($this, 'woo_vou_dashboard_widget_ajax_function'));
